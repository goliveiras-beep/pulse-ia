...
const CORES_PRIORIDADE_CALENDAR = { Alta: '11', Média: '5', Baixa: '10' };
...
export async function sincronizarTarefaCompasso(nome, tarefaId, tarefa) {
  try {
    ...
    await upsertEvento(accessToken, eventId, {
      ...
      start: { dateTime: isoBRT(dataPrazo, 9, 0), timeZone: 'America/Sao_Paulo' },
      end: { dateTime: isoBRT(dataPrazo, 9, 30), timeZone: 'America/Sao_Paulo' },
      colorId: CORES_PRIORIDADE_CALENDAR[tarefa.prioridade] || '5',
      // popup 1h antes e no dia anterior no mesmo horário — dá o "alarme" nativo do Google
      // (push no celular, se a pessoa tiver notificação da Agenda ativada).
