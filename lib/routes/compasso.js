...
function renderPage({ nome, isGestor, tarefas, equipeAtivos }) {
    ...
    const cardsHtml = itens.length ? itens.map(t => `
      <div class="card-tarefa" onclick="abrirTarefa('${esc(t.id)}')">
        ...
        <div class="card-footer">
          ${t.responsavel ? `<span class="card-resp"><span class="avatar-mini">${esc(iniciais(t.responsavel))}</span>${esc(t.responsavel.split(' ')[0])}</span>` : '<span class="card-resp" style="color:var(--muted)">Sem responsável</span>'}
          ${t.prazo ? `<span class="card-prazo">📅 ${esc(t.prazo)}</span>` : ''}
        </div>
      </div>`).join('') : `<div class="col-empty">Nenhuma tarefa aqui</div>`;
    return `
      <div class="coluna">
  ...
  const optsResp = equipeAtivos.map(n => `<option value="${esc(n)}">${esc(n)}</option>`).join('');
...
<div class="header">
  ...
  <div class="hr">
    ...
    <a href="/api/app" class="btn-sm">🏠 Início</a>
  </div>
</div>
<div class="wrap">
  <div class="top-bar">
...
<div class="modal-bg" id="modal-tarefa">
  <div class="modal">
    ...
    <select id="t-responsavel"><option value="">Sem responsável</option>${optsResp}</select>
    <label>Status</label>
