...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    // TEMPORÁRIO — reverte um id "APROVADO-..." de volta a pendente. Usado só pra corrigir
    // trocas de horário que foram indevidamente aprovadas pelo fluxo antigo. Remover depois.
    if (acao === 'reverter-pendente-temp') {
      const { idAtual, idOriginal } = req.body || {};
      if (!idAtual || !idOriginal) return res.status(400).json({ error: 'Dados inválidos' });
      const aus = await getSheet('Ausências!A2:F500');
      const aidx = aus.findIndex(r => r[0] === idAtual);
      if (aidx < 0) return res.status(404).json({ error: 'Não encontrado' });
      await setSheet(`Ausências!A${aidx+2}`, [[idOriginal]]);
      return res.status(200).json({ ok: true });
    }
