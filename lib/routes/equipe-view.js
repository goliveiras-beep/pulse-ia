...
import { sincronizarUmaPessoa } from '../google-calendar.js';
...
// ── Disponibilidade (restrição recorrente de horário — ex: aula de faculdade) ──────────
// Aba nova (31/07/2026): A=Nome, B=Dia da semana (Segunda..Domingo), C=Hora início (HH:MM),
// D=Hora fim (HH:MM), E=Motivo. Cada linha é UMA janela recorrente em que a pessoa não pode
// ser escalada. Consumida por api/escalas.js (mostra alerta se o turno bater em cima) e
// api/gerar-escala.js (evita gerar turno dentro da janela).
const DIAS_SEMANA = ['Domingo','Segunda','Terça','Quarta','Quinta','Sexta','Sábado'];
async function garantirAbaDisponibilidade() {
  const spreadsheet = await sheetsRequest(process.env.GOOGLE_SHEET_ID, '');
  const temAba = (spreadsheet.sheets||[]).some(s => s.properties.title === 'Disponibilidade');
  if (!temAba) {
    await sheetsRequest(process.env.GOOGLE_SHEET_ID, ':batchUpdate', 'POST', {
      requests: [{ addSheet: { properties: { title: 'Disponibilidade', gridProperties: { rowCount: 500, columnCount: 5 } } } }]
    });
    await setSheet('Disponibilidade!A1:E1', [['Nome', 'Dia da Semana', 'Hora Início', 'Hora Fim', 'Motivo']]);
  }
}
...
export default async function handler(req, res) {
  ...
  await garantirAbaDisponibilidade();
  const [equipeRaw, ausenciasRaw, disponibilidadeRaw] = await Promise.all([
  ...
  if (req.method === 'POST') {
    ...
    if (acao === 'aprovar-ausencia') {
      ...
      // Troca de horário não passa por aqui — marcar "aprovado" só troca o rótulo, não
      // executa a inversão real na Escala. Precisa ser aceita pelo colega (banner na home
      // dele), que aí sim já executa a troca (ver aceitar-troca em api/app.js).
      if (aus[aidx][2] === 'Troca de horário') return res.status(400).json({ error: 'Troca de horário não é aprovada pelo gestor — precisa ser aceita pelo colega envolvido.' });
      await setSheet(`Ausências!A${aidx+2}:F${aidx+2}`, [['APROVADO-'+id, aus[aidx][1], aus[aidx][2], aus[aidx][3], aus[aidx][4], aus[aidx][5]]]);
      // Precisa de await mesmo — serverless na Vercel não garante execução em segundo plano
      // depois da resposta ser enviada (a function pode ser finalizada antes de terminar).
      await sincronizarUmaPessoa(aus[aidx][1]);
      return res.status(200).json({ ok: true });
    }
    ...
    if (acao === 'adicionar-restricao') {
      const { nomeRestricao, diaSemana, horaInicio, horaFim, motivoRestricao } = req.body || {};
      if (!nomeRestricao || !DIAS_SEMANA.includes(diaSemana) || !horaInicio || !horaFim) {
        return res.status(400).json({ error: 'Preencha nome, dia da semana e os dois horários' });
      }
      await appendSheet('Disponibilidade!A:E', [[nomeRestricao, diaSemana, horaInicio, horaFim, motivoRestricao || '']]);
      return res.status(200).json({ ok: true });
    }

    if (acao === 'remover-restricao') {
      const { linhaRestricao } = req.body || {};
      const lr = parseInt(linhaRestricao);
      if (!lr) return res.status(400).json({ error: 'Linha inválida' });
      await setSheet(`Disponibilidade!A${lr}:E${lr}`, [['','','','','']]);
      return res.status(200).json({ ok: true });
    }
  ...
  // Restrições de disponibilidade (ex: horário de faculdade) — janelas recorrentes por dia da
  // semana em que a pessoa não pode ser escalada. Ver garantirAbaDisponibilidade() no topo.
  const restricoes = disponibilidadeRaw.map((r,i) => ({
    linha: i+2, nome: r[0]||'', diaSemana: r[1]||'', horaInicio: r[2]||'', horaFim: r[3]||'', motivo: r[4]||''
  })).filter(r => r.nome);
  const restricoesPorNome = {};
  restricoes.forEach(r => { (restricoesPorNome[r.nome] = restricoesPorNome[r.nome] || []).push(r); });
    ...
    <div style="background:var(--card);border:2px solid #d97706;border-radius:10px;padding:16px">
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px;flex-wrap:wrap">
        ...
        <div style="flex:1;min-width:140px">
          <div style="font-size:13px;font-weight:700;word-break:break-word">${esc(m.nome)}</div>
          <div style="font-size:11px;color:var(--text3);word-break:break-word">✉ ${esc(m.email)}</div>
        </div>
  ...
  function cardAtivo(m) {
    ...
    const nRestricoes = (restricoesPorNome[m.nome]||[]).length;
    const badgeRestricao = nRestricoes ? `<span style="background:#fef3c7;color:#92400e;border-radius:4px;padding:2px 7px;font-size:10px;font-weight:600">🕐 ${nRestricoes} ${nRestricoes>1?'restrições':'restrição'}</span>` : '';
    return `
    <div style="background:var(--card);border:1px solid var(--border);border-radius:10px;padding:14px 16px;display:flex;align-items:center;gap:12px;flex-wrap:wrap">
      ...
      <div style="flex:1;min-width:140px">
        <div style="font-size:14px;font-weight:700;color:var(--text);word-break:break-word">${esc(m.nome)}</div>
        <div style="font-size:11px;color:var(--text3);margin-top:2px;word-break:break-word">${esc(m.cargo)||'—'}${m.nucleo?' · '+esc(m.nucleo):''}</div>
        <div style="font-size:11px;color:var(--text3);margin-top:2px;word-break:break-word">${m.email ? '✉ '+esc(m.email) : '<span style="color:#f6ad55">⚠ Sem email</span>'}</div>
        ${m.telefone ? `<div style="font-size:11px;color:var(--text3);margin-top:1px">📞 ${esc(m.telefone)}</div>` : ''}
        ${badgeRestricao ? `<div style="margin-top:4px">${badgeRestricao}</div>` : ''}
      </div>
      <div style="display:flex;flex-direction:column;align-items:flex-end;gap:6px;flex-shrink:0">
        ...
        <div style="display:flex;gap:4px">
          ...
          <button onclick="abrirDisponibilidade(${m.linha})" style="background:none;border:1px solid var(--border);border-radius:5px;padding:3px 10px;font-size:11px;color:var(--text2);cursor:pointer">🕐</button>
        </div>
      </div>
    </div>`;
  }
  ...
  const membrosData = {};
  todos.filter(m=>m.nome).forEach(m => { membrosData[m.linha] = { ...m, restricoes: restricoesPorNome[m.nome]||[] }; });
  ...
  function renderSolicitacoes() {
    ...
    const cards = solicitacoesPendentes.map(s => {
      ...
      return '<div style="background:var(--card);border:1px solid '+c+';border-radius:10px;padding:12px 14px;display:flex;align-items:center;gap:12px;flex-wrap:wrap">'
...
@media(max-width:640px){
  ...
  #eq-relogio-brt{font-size:13px!important;font-weight:700!important}
...
<div class="header">
  ...
  <div class="hr">
    ...
    <button id="tt" class="btn-sm m-keep" onclick="(function(){var dk=document.documentElement.classList.toggle('dark');localStorage.setItem('pulse-theme',dk?'dark':'light');})()" style="font-size:14px;padding:3px 8px">🌙</button>
    <div class="m-keep" style="position:relative">
      ...
      <div id="menu-dropdown" style="display:none;position:absolute;top:calc(100% + 8px);right:0;background:var(--card);border:1px solid var(--border);border-radius:8px;box-shadow:0 8px 24px rgba(0,0,0,.35);min-width:210px;overflow:hidden;z-index:200">
        ...
        <a href="/api/maestro-booking-dia" class="menu-item">&#128197; Booking — Grade do dia</a>
...
<div class="wrap">
  ...
  <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:14px">
    ...
    <button onclick="abrirExportarCsv()" style="border:1px solid var(--border);border-radius:8px;padding:8px 12px;font-size:12px;font-weight:600;background:var(--bg2);color:var(--text);cursor:pointer">📤 Exportar CSV</button>
  </div>
...
<div class="modal-bg" id="modal-disp">
  <div class="modal">
    <h3>Disponibilidade — <span id="disp-nome"></span></h3>
    <input type="hidden" id="disp-linha">
    <div style="font-size:11px;color:var(--text3);margin-bottom:10px">Janelas recorrentes em que essa pessoa NÃO pode ser escalada (ex: horário de faculdade). Aparece como alerta na Escala e é evitado ao gerar escala por IA.</div>
    <div id="disp-lista" style="display:flex;flex-direction:column;gap:6px;margin-bottom:16px"></div>
    <div class="grid2">
      <div class="field"><label>Dia da semana</label>
        <select id="disp-dia">${DIAS_SEMANA.map(d=>`<option value="${esc(d)}">${esc(d)}</option>`).join('')}</select>
      </div>
      <div class="field"><label>Motivo</label><input type="text" id="disp-motivo" placeholder="Ex: Faculdade"></div>
      <div class="field"><label>Início</label><input type="text" id="disp-inicio" placeholder="HH:MM"></div>
      <div class="field"><label>Fim</label><input type="text" id="disp-fim" placeholder="HH:MM"></div>
    </div>
    <div class="modal-btns">
      <button class="btn-cancel" onclick="fecharModalDisp()">Fechar</button>
      <button class="btn-primary" onclick="adicionarRestricao()">+ Adicionar janela</button>
    </div>
  </div>
</div>
...
<div class="modal-bg" id="modal-export">
  <div class="modal">
    <h3>Exportar CSV</h3>
    <div style="font-size:11px;color:var(--text3);margin-bottom:10px">Escolha os cargos que devem entrar no arquivo. Por padrão exporta Nome, Cargo, Núcleo, Email, Telefone, Perfil e Tipo de contrato — sem CPF/RG/endereço.</div>
    <div style="display:flex;gap:8px;margin-bottom:8px">
      <button class="btn-cancel" style="flex:1" onclick="marcarTodosCargos(true)">Marcar todos</button>
      <button class="btn-cancel" style="flex:1" onclick="marcarTodosCargos(false)">Desmarcar todos</button>
    </div>
    <div id="export-cargos-lista" style="display:flex;flex-direction:column;gap:6px;max-height:220px;overflow-y:auto;margin-bottom:16px;border:1px solid var(--border);border-radius:8px;padding:8px"></div>
    <div class="modal-btns">
      <button class="btn-cancel" onclick="fecharModalExport()">Cancelar</button>
      <button class="btn-primary" onclick="exportarCsv()">📤 Exportar</button>
    </div>
  </div>
</div>
...
<script>
// Só os campos não sensíveis — CPF/RG/nascimento/endereço nunca chegam ao navegador aqui,
// pra exportar CSV não correr o risco de expor esse dado num arquivo que pode circular por email/Drive.
const EQUIPE_EXPORT = ${JSON.stringify(ativos.map(m => ({
  nome: m.nome, cargo: m.cargo, nucleo: m.nucleo, email: m.email,
  telefone: m.telefone, perfil: m.perfil, tipoContrato: m.tipoContrato,
})))};
...
var _linhaDispAtual = null;
function renderListaRestricoes(m){
  var lista = document.getElementById('disp-lista');
  if(!m.restricoes || !m.restricoes.length){
    lista.innerHTML = '<div style="font-size:12px;color:var(--text3)">Nenhuma janela cadastrada ainda.</div>';
    return;
  }
  lista.innerHTML = m.restricoes.map(function(r){
    return '<div style="display:flex;align-items:center;gap:8px;background:var(--bg2);border-radius:6px;padding:6px 10px;font-size:12px">'
      +'<div style="flex:1"><b>'+r.diaSemana+'</b> '+r.horaInicio+'–'+r.horaFim+(r.motivo?' <span style="color:var(--text3)">('+r.motivo+')</span>':'')+'</div>'
      +'<button onclick="removerRestricao('+r.linha+')" style="background:none;border:1px solid #dc2626;border-radius:5px;padding:2px 8px;font-size:11px;color:#dc2626;cursor:pointer">✕</button>'
      +'</div>';
  }).join('');
}
function abrirDisponibilidade(linha){
  var m=_membrosData[linha];
  if(!m)return;
  _linhaDispAtual = linha;
  document.getElementById('disp-linha').value = linha;
  document.getElementById('disp-nome').textContent = m.nome;
  document.getElementById('disp-motivo').value = '';
  document.getElementById('disp-inicio').value = '';
  document.getElementById('disp-fim').value = '';
  renderListaRestricoes(m);
  document.getElementById('modal-disp').classList.add('open');
}
function fecharModalDisp(){document.getElementById('modal-disp').classList.remove('open');}
document.getElementById('modal-disp').addEventListener('click',e=>{if(e.target===e.currentTarget)fecharModalDisp();});

function escHtmlExport(s){ return String(s||'').replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/</g,'&lt;').replace(/>/g,'&gt;'); }
function abrirExportarCsv(){
  var cargos = Array.from(new Set(EQUIPE_EXPORT.map(function(m){ return m.cargo || '(sem cargo)'; }))).sort(function(a,b){ return a.localeCompare(b,'pt-BR'); });
  var lista = document.getElementById('export-cargos-lista');
  lista.innerHTML = cargos.map(function(c){
    return '<label style="display:flex;align-items:center;gap:8px;font-size:13px;font-weight:400;margin:0;cursor:pointer"><input type="checkbox" class="export-cargo-chk" value="'+escHtmlExport(c)+'" checked style="width:auto;margin:0"> '+escHtmlExport(c)+'</label>';
  }).join('');
  document.getElementById('modal-export').classList.add('open');
}
function fecharModalExport(){ document.getElementById('modal-export').classList.remove('open'); }
document.getElementById('modal-export').addEventListener('click',e=>{if(e.target===e.currentTarget)fecharModalExport();});
function marcarTodosCargos(v){
  document.querySelectorAll('.export-cargo-chk').forEach(function(c){ c.checked = v; });
}
...
function exportarCsv(){
  var selecionados = Array.from(document.querySelectorAll('.export-cargo-chk:checked')).map(function(c){ return c.value; });
  if (!selecionados.length){ toast('Selecione ao menos um cargo', '#dc2626'); return; }
  var linhas = EQUIPE_EXPORT.filter(function(m){ return selecionados.indexOf(m.cargo || '(sem cargo)') !== -1; });
  var cabecalho = ['Nome','Cargo','Núcleo','Email','Telefone','Perfil','Tipo de Contrato'];
  var corpo = linhas.map(function(m){
    return [m.nome, m.cargo, m.nucleo, m.email, m.telefone, m.perfil, m.tipoContrato].map(csvEscape).join(',');
  });
  var csv = '﻿' + [cabecalho.join(','), ...corpo].join('\\r\\n');
  var blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
  var url = URL.createObjectURL(blob);
  var a = document.createElement('a');
  var hoje = new Date().toISOString().slice(0,10);
  a.href = url;
  a.download = 'equipe_pulse_'+hoje+'.csv';
...
async function adicionarRestricao(){
  var m=_membrosData[_linhaDispAtual];
  if(!m)return;
  var d=await post({
    acao:'adicionar-restricao',
    nomeRestricao:m.nome,
    diaSemana:document.getElementById('disp-dia').value,
    horaInicio:document.getElementById('disp-inicio').value,
    horaFim:document.getElementById('disp-fim').value,
    motivoRestricao:document.getElementById('disp-motivo').value,
  });
  if(d.ok){toast('Janela adicionada!');setTimeout(()=>location.reload(),900);}
  else toast('Erro: '+d.error,'#dc2626');
}
async function removerRestricao(linhaRestricao){
  if(!confirm('Remover esta janela de indisponibilidade?'))return;
  var d=await post({acao:'remover-restricao',linhaRestricao});
  if(d.ok){toast('Removida.');setTimeout(()=>location.reload(),900);}
  else toast('Erro: '+d.error,'#dc2626');
}
