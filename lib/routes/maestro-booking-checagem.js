    ...
    <div style="margin-top:6px;padding-top:6px;border-top:1px dashed var(--border2)">
      ...
      ${emails.map((e) => `
        <div style="padding:4px 0;font-size:10px;color:var(--text3)">
          ...
          ${e.arquivado ? `<div style="color:var(--green-m-v)">✅ Arquivado</div>` : ''}
          ${e.erroArquivamento ? `<div style="color:var(--red-m-v)">⚠️ Falha ao arquivar: ${esc(e.erroArquivamento)}</div>` : ''}
          ${e.pendenteDeArquivamento ? `<div style="color:var(--text4)">⏳ Arquivamento pendente (fica pra próxima execução)</div>` : ''}
        </div>`).join('')}
    </div>`;
}
