...
function renderCartaoOk(e, i) {
  ...
  return `
    <div style="border:1px solid ${cores.border};border-radius:8px;margin-bottom:8px;overflow:hidden">
      <div style="background:${cores.bg};padding:8px 12px;display:flex;align-items:center;gap:10px;cursor:${temFicha ? 'pointer' : 'default'}" ${temFicha ? `onclick="var el=document.getElementById('${idFicha}');el.style.display=el.style.display==='none'?'block':'none';"` : ''}>
        ...
        <div style="font-size:10px;font-weight:700;color:${cores.texto}">OK${temFicha ? ' 🔧' : ''}</div>
      </div>
  ...
  return `
    ...
    <div class="card">
      <div class="card-body" style="text-align:center;color:var(--text3);font-size:12px">
        A checagem de um dia especifico so avisa e arquiva e-mail relacionado - nao ha
        execucao automatica ainda pra dias futuros, precisa navegar (‹ ›) pra conferir.
      </div>
    </div>`;
}
  ...
  return `
    ...
    <div class="card">
      <div class="card-header" style="display:flex;align-items:center;gap:6px">
        ...
        <div style="flex:1;text-align:center" id="checagemLabel">${renderLabelDia(r.dataAirtable, r.resumo.totalAnalisados)}</div>
    ...
    <script>
      ...
      async function navChecagemDia(dir) {
        var novaData = checagemSomarDias(_checagemDiaAtual, dir);
  ...
  if (req.query.formato === 'json') {
    ...
    try {
      const resultado = usaDiaEspecifico
        ? await checarPendenciasBooking(null, dataParam)
