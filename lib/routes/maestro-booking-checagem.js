...
import { checarPendenciasBooking, debugBookingsPorCompeticao, debugTestarEscrita } from '../booking/checagem.js';
...
export default async function handler(req, res) {
  ...
  if (req.query.debugCompeticao) {
    res.setHeader('Content-Type', 'application/json; charset=utf-8');
    return res.status(200).json(await debugBookingsPorCompeticao(req.query.debugCompeticao));
  }

  if (req.query.debugTesteEscrita) {
    res.setHeader('Content-Type', 'application/json; charset=utf-8');
    const [recordId, campo, valor] = req.query.debugTesteEscrita.split('|');
    return res.status(200).json(await debugTestarEscrita(recordId, campo, valor));
  }
