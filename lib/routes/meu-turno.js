...
async function getEventosDia(dataStr) {
  ...
  try {
    ...
    return (d.records||[]).map(r=>({nome:r.fields['Match ID']||'Evento',hora:toHoraBRT(r.fields['PGM (horário)']||''),tipo:r.fields['Tipo de Conteúdo']||''}))
...
@media(max-width:640px){
  ...
  .hts{display:none!important}
  .dgrid{gap:4px!important}
  .dcell{padding:6px 3px!important;min-height:58px!important}
}
`;
}
  ...
  return `
    ...
    <div style="position:relative">
      ...
      <div id="menu-dropdown" style="display:none;position:absolute;top:calc(100% + 8px);right:0;background:var(--card);border:1px solid var(--border);border-radius:8px;box-shadow:0 8px 24px rgba(0,0,0,.35);min-width:210px;overflow:hidden;z-index:200">
        ...
        <a href="/api/compasso" class="menu-item">&#129517; Compasso</a>${itensGestor}
...
${MENU_SCRIPT}
  ...
  function renderTurnoCard(turno, ausencia, label, isD1=false) {
    ...
    return `<div class="card" style="background:${bg};border-color:${bc};padding:10px 14px;min-width:0"><div style="font-size:10px;color:${isD1?'var(--blue-m-v)':'var(--text3)'};font-weight:600;text-transform:uppercase;margin-bottom:4px">${esc(label)}</div><div style="font-size:22px;font-weight:700;color:${tc};word-break:break-word">${esc(turno[3])} → ${esc(turno[4])}</div>${eventosMap[isD1?d1Str:hojeStr]?`<div style="margin-top:6px">${eventosMap[isD1?d1Str:hojeStr].map(e=>`<div style="font-size:11px;color:var(--text2);padding:2px 0;word-break:break-word">${e.hora?esc(e.hora)+' · ':''}${esc(e.nome)}</div>`).join('')}</div>`:''}</div>`;
  }
  ...
  semanas.forEach((dias,si)=>{
    ...
    semanasHTML+=`<div style="margin-bottom:20px">
      ...
      <div class="dgrid" style="display:grid;grid-template-columns:repeat(7,minmax(0,1fr));gap:6px">
        ${dias.map(d=>{
          const df=fmtData(d);
          ...
          return `<div class="dcell" style="background:${bg};border:1px solid ${bc};border-radius:8px;padding:8px 6px;text-align:center;min-height:70px;min-width:0;overflow:hidden">
            ...
            <div style="font-size:10px;font-weight:700;color:${turnoColor};margin-top:4px;word-break:break-word">${esc(turnoTxt)}</div>
...
<div class="header">
  ...
  <div class="hts" style="margin-left:auto;font-size:10px;color:#888;white-space:nowrap">${atualizado}</div>
  <div class="hr">${menuHTML(isGestor)}</div>
</div>
<div class="wrap">
  <div class="card" style="display:flex;align-items:center;gap:12px;margin-bottom:20px;padding:14px 16px;flex-wrap:wrap">
    <div class="av-big" style="flex-shrink:0">${iniciais(nome)}</div>
    <div style="flex:1;min-width:140px">
      <div style="font-size:16px;font-weight:700;word-break:break-word">${esc(nome)}</div>
      <div style="font-size:12px;color:var(--text3);word-break:break-word">${esc(cargo||'Colaborador')} · ${esc(nucleo)}</div>
    </div>
  </div>

  <div class="hgrid" style="display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:10px;margin-bottom:20px">
