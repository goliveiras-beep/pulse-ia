...
import { exigirSessaoDeGestor } from '../setup-guard.js';
...
export default async function handler(req, res) {
  if (!(await exigirSessaoDeGestor(req, res))) return;
  ...
  try {
    ...
    // Expande a grade pra 15 colunas (A:O) antes de escrever em L:O, senão a API rejeita
    // escrita fora dos limites atuais — mesmo motivo do bug corrigido no Tipo de Parque.
    const spreadsheet = await sheetsRequest(SHEET_ID, '');
    const eqSheetMeta = spreadsheet.sheets?.find(s => s.properties.title === 'Equipamentos');
    const colAtual = eqSheetMeta?.properties.gridProperties?.columnCount || 11;
    if (eqSheetMeta && colAtual < 15) {
      await sheetsRequest(SHEET_ID, ':batchUpdate', 'POST', {
        requests: [{
          updateSheetProperties: {
            properties: { sheetId: eqSheetMeta.properties.sheetId, gridProperties: { columnCount: 15 } },
            fields: 'gridProperties.columnCount'
          }
        }]
      });
      log.push('✓ Grade expandida para 15 colunas (A:O)');
    }
