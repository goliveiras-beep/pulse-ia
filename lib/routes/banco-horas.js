...
import { analisarEscala } from '../escalas-engine.js';
import { calcularDia, horasNoturnas, jornadaContratada, feriadosDoAno, topUpSemanal40h, LIMITE_BANCO_CLT_PJ, JORNADA_SEMANAL_CLT_PJ } from '../horas-extras-engine.js';
...
// Regras de banco de horas / hora extra por tipo de contrato (domingo/feriado/7º dia consecutivo,
// teto semanal de 40h pra Live Mode/PJ, adicional noturno) vivem em lib/horas-extras-engine.js —
// importadas no topo do arquivo, pra api/gerar-escala.js poder reusar a mesma regra sem duplicar.
...
export default async function handler(req, res) {
  ...
  // Ausências buscada só pra alimentar analisarEscala() (detecção de 7º dia consecutivo) — mesma
  // função/limiar já usado em escalas.js, pra não ter duas definições divergentes de "dia consecutivo".
  ...
  // Datas do período em "DD/MM", pra alimentar analisarEscala() (ela mesma olha pra trás até 14 dias
  // dentro de escalaRaw pra achar sequência de dias consecutivos, então não precisa de margem aqui).
  const datasPeriodo = [];
  for (let data = new Date(dataIni); data <= dataFim; data.setDate(data.getDate()+1)) datasPeriodo.push(fmtData(data));
  const alertasPorPessoa = analisarEscala(escalaRaw, ausenciasRaw, equipe.map(p => p.nome), datasPeriodo);

  const resultado = equipe.map(p => {
    let diasTrabalhados = 0, horasTotais = 0, bancoTotal = 0, extra50Total = 0, extra100Total = 0, noturnoTotal = 0, diasFolga = 0;
    const dias = [];
    const diaCalcs = []; // todo dia trabalhado, mesmo sem excedente — só usado pro teto semanal de 40h (CLT/PJ)
    for (let data = new Date(dataIni); data <= dataFim; data.setDate(data.getDate()+1)) {
      ...
      if (reg && (reg[5] === 'Folga' || reg[5] === 'Folga/Ausente')) { diasFolga++; continue; }
      if (!reg || !reg[3] || !reg[4]) continue;
      const durBruta = duracaoHoras(reg[3], reg[4]);
      if (durBruta <= 0) continue;
      const isDomingo = data.getDay() === 0;
      const isFeriado = feriadosDoAno(data.getFullYear()).has(df);
      const alertasDia = alertasPorPessoa[p.nome]?.[df]?.alertas || [];
      const isSetimoDia = alertasDia.some(a => a.codigo === 'CONSECUTIVOS');
      const semBanco = isDomingo || isFeriado || isSetimoDia;
      const motivo100 = isDomingo ? 'dom' : isFeriado ? 'feriado' : isSetimoDia ? '7º dia' : '';
      const calc = calcularDia(durBruta, p.tipoContrato, semBanco);
      const noturno = horasNoturnas(reg[3], reg[4]) * 0.20; // adicional noturno = 20% sobre a hora trabalhada nesse período
      diasTrabalhados++;
      ...
      diaCalcs.push({ data: new Date(data), semBanco, trabalhadas: calc.trabalhadas, banco: calc.banco });
      if (calc.banco > 0 || calc.extra50 > 0 || calc.extra100 > 0 || noturno > 0) {
        dias.push({ df, isDomingo, motivo100, noturno, ...calc });
      }
    }
    // Teto semanal de 40h — só Live Mode/PJ; LET segue só a regra diária (6x1), sem alteração.
    if (p.tipoContrato === 'CLT' || p.tipoContrato === 'PJ') {
      const topUp = topUpSemanal40h(diaCalcs);
      extra100Total += topUp;
    }
    const horasFolgaTotal = diasFolga * jornadaContratada(p.tipoContrato);
    return { ...p, diasTrabalhados, horasTotais, bancoTotal, extra50Total, extra100Total, noturnoTotal, diasFolga, horasFolgaTotal, dias };
  });
  ...
  const maiorConsolidado = Math.max(1, ...resultado.map(p => p.extra50Total + p.extra100Total + p.noturnoTotal));
  ...
  const cardsHtml = resultado.length ? resultado.map(p => {
    ...
    // Live Mode (CLT) e PJ: card consolidado, sem lista dia-a-dia — só o total de horas
    // extras (50%+100%+noturno) numa linha única. LET (Temporário) continua com o detalhe
    // completo, sem alterações.
    const consolidarExtras = p.tipoContrato === 'CLT' || p.tipoContrato === 'PJ';
    const extrasConsolidadoTotal = p.extra50Total + p.extra100Total + p.noturnoTotal;
    const pctConsolidado = Math.round((extrasConsolidadoTotal / maiorConsolidado) * 100);
    const diasDetalheHtml = (!consolidarExtras && p.dias.length) ? `
      <div style="margin-top:10px;border-top:1px solid var(--border);padding-top:8px;display:flex;flex-direction:column;gap:4px;max-height:140px;overflow-y:auto">
        ${p.dias.map(d => `<div style="display:flex;align-items:center;gap:8px;font-size:11px;flex-wrap:wrap">
          <span style="min-width:40px;color:var(--text3);font-weight:600">${esc(d.df)}${d.motivo100?` <span style=\"color:#7c3aed\">(${esc(d.motivo100)})</span>`:''}</span>
    ...
    const folgasLinhaHtml = `
      <div style="display:flex;justify-content:space-between;font-size:11px;margin-bottom:6px"><span style="color:#94a3b8;font-weight:600">Folgas no período</span><span style="font-weight:700;color:var(--text)">${p.diasFolga} dia${p.diasFolga===1?'':'s'} · ${fmtH(p.horasFolgaTotal)}</span></div>`;
    // Saldo = horas extras (50%+100%+noturno) já feitas menos as horas de folga tiradas no período —
    // só faz sentido pra Live Mode/PJ, que é onde consolidamos as extras numa linha só (ver acima).
    const saldoExtrasFolgas = extrasConsolidadoTotal - p.horasFolgaTotal;
    // Dias de folga equivalentes ao saldo, convertendo pela jornada contratada (8h) — arredondado pra
    // cima, já que folga é sempre em dia inteiro (meio dia de saldo já exige o dia cheio de folga).
    const diasParaAbater = Math.ceil(Math.abs(saldoExtrasFolgas) / jornadaContratada(p.tipoContrato));
    const saldoLinhaHtml = consolidarExtras ? `
      <div style="display:flex;justify-content:space-between;font-size:11px;margin-top:6px;padding-top:6px;border-top:1px dashed var(--border)"><span style="color:var(--text2);font-weight:600">Saldo (extras − folgas)</span><span style="font-weight:700;color:${saldoExtrasFolgas>=0?'#d97706':'#16a34a'}">${saldoExtrasFolgas>=0?'+':'-'}${fmtH(Math.abs(saldoExtrasFolgas))}</span></div>
      <div style="text-align:right;font-size:10px;color:var(--text3);margin-top:2px">${saldoExtrasFolgas>=0?`≈ ${diasParaAbater} dia${diasParaAbater===1?'':'s'} de folga p/ abater`:`${diasParaAbater} dia${diasParaAbater===1?'':'s'} tirado${diasParaAbater===1?'':'s'} a mais`}</div>` : '';
    const extrasBlocoHtml = consolidarExtras ? `
      <div>
        <div style="display:flex;justify-content:space-between;font-size:11px;margin-bottom:3px"><span style="color:#f6ad55;font-weight:600">Horas extras (total)</span><span style="font-weight:700;color:var(--text)">${fmtH(extrasConsolidadoTotal)}</span></div>
        <div style="background:var(--bg3);border-radius:4px;height:8px;overflow:hidden"><div style="background:#d97706;height:100%;width:${pctConsolidado}%"></div></div>
        ${saldoLinhaHtml}
      </div>` : `
    ...
    <div class="colab-card" data-tipo="${esc(p.tipoContrato)}" data-banco="${p.bancoTotal}" data-extra50="${p.extra50Total}" data-extra100="${p.extra100Total}" data-noturno="${p.noturnoTotal}" style="background:var(--card);border:1px solid var(--border);border-radius:10px;padding:16px">
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px">
        ...
        <div style="flex:1;min-width:0">
          <div style="font-size:13px;font-weight:700;color:var(--text);word-break:break-word">${esc(p.nome)}</div>
          <div style="font-size:10px;color:var(--text3);word-break:break-word">${esc(p.cargo)||'—'}</div>
        </div>
...
<div style="max-width:1300px;margin:0 auto;padding:18px 20px">
  <div style="background:var(--card);border:1px solid var(--border);border-radius:10px;padding:10px 16px;margin-bottom:16px;font-size:11px;color:var(--text2);line-height:1.6">
    <b>Regras aplicadas</b> · <span style="color:#92400e;font-weight:600">LET (6x1, 6h/dia):</span> dia normal, até 2h excedentes vão para o banco de horas; depois disso é <span style="color:#16a34a">hora extra 50%</span>.
    <span style="color:#1d4ed8;font-weight:600;margin-left:8px">Live Mode e PJ (8h/dia, contrato de ${JORNADA_SEMANAL_CLT_PJ}h/semana):</span> dia normal, até ${LIMITE_BANCO_CLT_PJ}h excedentes por dia vão para o banco de horas; depois disso é hora extra 100%. O que passar de ${JORNADA_SEMANAL_CLT_PJ}h na semana também é <span style="color:#d97706">hora extra 100%</span>, mesmo em dias que isoladamente não bateram 8h.
    <span style="color:#d97706;font-weight:600;margin-left:8px">Domingo, feriado nacional ou 7º dia consecutivo sem folga:</span> pra qualquer tipo de contrato, sem banco — toda hora excedente é <span style="color:#d97706">hora extra 100%</span>.
