...
export default async function handler(req, res) {
  ...
  // Mês selecionado via ?offset=N (relativo ao mês atual) e modo via ?modo=mes|acumulado
  const offset = parseInt(req.query.offset || '0') || 0;
  const modo = req.query.modo === 'acumulado' ? 'acumulado' : 'mes';
  ...
  // Período de cálculo: no modo "mês" é só o mês selecionado; no "acumulado" é de 1º de janeiro
  // do ano selecionado até o fim do mês selecionado (a coluna A da Escala é DD/MM sem ano, então
  // o acumulado sempre fica restrito a um único ano pra não colidir datas de anos diferentes)
  const dataInicioCalc = modo === 'acumulado' ? new Date(ano, 0, 1) : new Date(ano, mes, 1);
  const dataFimCalc = new Date(ano, mes, ultimoDia);
  const totalDiasPeriodo = Math.round((dataFimCalc - dataInicioCalc) / 86400000) + 1;

  // Índice pra lookup O(1) em vez de escalaRaw.find() repetido — importante no modo acumulado (até ~365 dias × pessoa)
  const escalaIndex = new Map();
  escalaRaw.forEach(r => { if (r[0] && r[2]) escalaIndex.set(r[0]+'|'+normalizarNome(r[2]), r); });
  ...
  const resultado = equipe.map(p => {
    ...
    const dias = [];
    const porMes = {}; // mesIdx -> {banco, extra} — usado no gráfico do modo acumulado
    const nomeNorm = normalizarNome(p.nome);
    for (let i = 0; i < totalDiasPeriodo; i++) {
      const data = new Date(dataInicioCalc);
      data.setDate(data.getDate() + i);
      const df = fmtData(data);
      const reg = escalaIndex.get(df+'|'+nomeNorm);
      ...
      if (calc.banco > 0 || calc.extra100 > 0) {
        ...
        const mi = data.getMonth();
        if (!porMes[mi]) porMes[mi] = { banco: 0, extra: 0 };
        porMes[mi].banco += calc.banco;
        porMes[mi].extra += calc.extra100;
      }
    }
    return { ...p, diasTrabalhados, horasTotais, bancoTotal, extraTotal, dias, porMes };
  });
  ...
  // Só faz sentido pro modo "mês" (calendário de dias-da-semana de um único mês) — no modo
  // acumulado (ano inteiro) é pulado por completo, o que também evita uma chamada extra à Airtable.
  let diaBarHtml = '', diasComDados = [], melhoresParaFolga = [], diasSobrecarregados = [], fluxoMaisFraco = [];
  if (modo === 'mes') {
    ...
    diasComDados = weekdayAnalise.filter(w => w.diasNoMes > 0);
    melhoresParaFolga = [...diasComDados].sort((a,b) => b.folgaScore - a.folgaScore).slice(0, 2);
    diasSobrecarregados = [...diasComDados].sort((a,b) => a.folgaScore - b.folgaScore).slice(0, 2);
    fluxoMaisFraco = [...diasComDados].sort((a,b) => a.mediaEventos - b.mediaEventos).slice(0, 2);
    ...
    diaBarHtml = weekdayAnalise.map(w => {
  ...
  // ── Gráfico de barras: por dia (modo mês) ou por mês (modo acumulado) ─────
  let barrasData;
  if (modo === 'acumulado') {
    const porMesAgregado = {};
    resultado.forEach(p => Object.entries(p.porMes||{}).forEach(([mi, v]) => {
      mi = parseInt(mi, 10);
      if (!porMesAgregado[mi]) porMesAgregado[mi] = { banco: 0, extra: 0 };
      porMesAgregado[mi].banco += v.banco;
      porMesAgregado[mi].extra += v.extra;
    }));
    barrasData = [];
    for (let mi = 0; mi <= mes; mi++) {
      const v = porMesAgregado[mi] || { banco: 0, extra: 0 };
      barrasData.push({ label: MESES_PT[mi].slice(0,3), banco: v.banco, extra: v.extra });
    }
  } else {
    const porDiaAgregado = {};
    resultado.forEach(p => p.dias.forEach(d => {
      if (!porDiaAgregado[d.df]) porDiaAgregado[d.df] = { banco: 0, extra: 0 };
      porDiaAgregado[d.df].banco += d.banco;
      porDiaAgregado[d.df].extra += d.extra100;
    }));
    barrasData = [];
    for (let d = 1; d <= ultimoDia; d++) {
      const df = fmtData(new Date(ano, mes, d));
      const v = porDiaAgregado[df] || { banco: 0, extra: 0 };
      barrasData.push({ label: String(d), banco: v.banco, extra: v.extra });
    }
  }
  const maiorBarra = Math.max(1, ...barrasData.map(b => b.banco + b.extra));
  const ALTURA_BARRA = 90;
  const barrasHtml = barrasData.map(b => {
    const total = b.banco + b.extra;
    const pxBanco = Math.round((b.banco / maiorBarra) * ALTURA_BARRA);
    const pxExtra = Math.round((b.extra / maiorBarra) * ALTURA_BARRA);
    return `<div style="display:flex;flex-direction:column;align-items:center;gap:3px;flex:1;min-width:0">
      <div style="width:100%;max-width:26px;height:${ALTURA_BARRA}px;display:flex;flex-direction:column-reverse;background:var(--bg3);border-radius:3px 3px 0 0;overflow:hidden" title="${esc(b.label)}: ${fmtH(total)}">
        <div style="width:100%;background:#1d4ed8;height:${pxBanco}px"></div>
        <div style="width:100%;background:#d97706;height:${pxExtra}px"></div>
      </div>
      <span style="font-size:9px;color:var(--text3)">${esc(b.label)}</span>
    </div>`;
  }).join('');
  const barrasSectionHtml = `
  <div style="background:var(--card);border:1px solid var(--border);border-radius:10px;padding:16px;margin-bottom:18px">
    <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.05em;color:var(--text3);margin-bottom:10px">📊 Banco de horas + hora extra ${modo==='acumulado'?'por mês':'por dia'} <span style="color:#1d4ed8">■ banco</span> <span style="color:#d97706">■ extra 100%</span></div>
    <div style="display:flex;align-items:flex-end;gap:4px;overflow-x:auto;padding-bottom:4px">${barrasHtml}</div>
  </div>`;

  // ── Gráfico por pessoa: ranking do maior pro menor ────────────────────────
  const pessoasOrdenadas = [...resultado].filter(p => p.bancoTotal>0 || p.extraTotal>0).sort((a,b) => (b.bancoTotal+b.extraTotal) - (a.bancoTotal+a.extraTotal));
  const maiorPessoaTotal = Math.max(1, ...pessoasOrdenadas.map(p => p.bancoTotal+p.extraTotal));
  const chartPessoasHtml = pessoasOrdenadas.length ? pessoasOrdenadas.map(p => {
    const total = p.bancoTotal + p.extraTotal;
    const pctBanco = (p.bancoTotal / maiorPessoaTotal) * 100;
    const pctExtra = (p.extraTotal / maiorPessoaTotal) * 100;
    return `<div style="display:flex;align-items:center;gap:8px;margin-bottom:7px">
      <span style="width:140px;flex-shrink:0;font-size:11px;color:var(--text2);white-space:nowrap;overflow:hidden;text-overflow:ellipsis" title="${esc(p.nome)}">${esc(p.nome.split(' ').slice(0,2).join(' '))}</span>
      <div style="flex:1;display:flex;height:14px;border-radius:4px;overflow:hidden;background:var(--bg3)">
        ${p.bancoTotal>0?`<div style="width:${pctBanco}%;background:#1d4ed8" title="Banco: ${fmtH(p.bancoTotal)}"></div>`:''}
        ${p.extraTotal>0?`<div style="width:${pctExtra}%;background:#d97706" title="Extra: ${fmtH(p.extraTotal)}"></div>`:''}
      </div>
      <span style="width:60px;flex-shrink:0;font-size:11px;font-weight:700;color:var(--text);text-align:right">${fmtH(total)}</span>
    </div>`;
  }).join('') : '<div style="color:var(--text3);font-size:12px;padding:10px 0">Nenhum banco de horas ou hora extra no período.</div>';
  const chartPessoasSectionHtml = `
  <div style="background:var(--card);border:1px solid var(--border);border-radius:10px;padding:16px;margin-bottom:18px">
    <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.05em;color:var(--text3);margin-bottom:10px">🏆 Ranking por pessoa (banco <span style="color:#1d4ed8">■</span> + extra <span style="color:#d97706">■</span>)</div>
    ${chartPessoasHtml}
  </div>`;
...
<div id="bh-header" style="background:var(--header);padding:12px 20px;display:flex;align-items:center;gap:10px;position:sticky;top:0;z-index:100">
  ...
  <div style="min-width:0"><div id="bh-title" style="font-size:14px;font-weight:600;color:#fff">Pulse — Banco de horas &amp; Horas extras</div><div id="bh-sub" style="font-size:11px;color:#999;text-transform:capitalize">${modo==='acumulado' ? `Acumulado ${ano} até ${nomeMes}` : `${nomeMes} ${ano}`} · baseado na escala planejada</div></div>
  <div id="bh-header-right" style="margin-left:auto;display:flex;align-items:center;gap:6px">
    ...
    <div style="display:flex;background:rgba(255,255,255,.06);border-radius:6px;padding:2px;gap:2px">
      <a href="/api/banco-horas?offset=${offset}&modo=mes" class="btn-sm" style="border:none;${modo==='mes'?'background:var(--card);color:var(--text)':'color:#a0aec0'}">Mês</a>
      <a href="/api/banco-horas?offset=${offset}&modo=acumulado" class="btn-sm" style="border:none;${modo==='acumulado'?'background:var(--card);color:var(--text)':'color:#a0aec0'}">Acumulado</a>
    </div>
    <a href="/api/banco-horas?offset=${offset-1}&modo=${modo}" class="btn-sm" title="Mês anterior"><span class="bh-txt-full">&#8249; mês anterior</span><span class="bh-txt-short">&#8249;</span></a>
    ...
    <a href="/api/banco-horas?offset=${offset+1}&modo=${modo}" class="btn-sm" title="Próximo mês"><span class="bh-txt-full">próximo mês &#8250;</span><span class="bh-txt-short">&#8250;</span></a>
    <a href="/api/banco-horas?offset=${offset}&modo=${modo}" class="btn-sm" style="background:#1a2744;border-color:#2a4080;color:#63b3ed"><span class="bh-txt-full">&#128202; Gerar relatório</span><span class="bh-txt-short">&#128202;</span></a>
...
var HOJE_ANO=${hoje.getFullYear()}, HOJE_MES=${hoje.getMonth()}, MODO_ATUAL='${modo}';
function irParaMes(){
  ...
  location.href='/api/banco-horas?offset='+offsetCalc+'&modo='+MODO_ATUAL;
}
function fmtHJs(h){
  if (h===0) return '0h';
