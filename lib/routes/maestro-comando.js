...
export default async function handler(req, res) {
  ...
  // getPulseSession só devolve {nome} (reaproveita o cookie do Pulse, sem perfil) - checagem
  // extra aqui porque o comando é executado de verdade no encoder pelo agente local, inclusive
  // durante transmissão em andamento (achado alto do SentinelaMODE 2026-09-14).
  try {
    const eq = await sheetsRequest(SHEET_ID, '/values/Equipe!A2:I200').then(d => d.values || []);
    const u = eq.find(r => r[0] === session.nome);
    if (u?.[8] !== 'gestor') return res.redirect(302, '/api/maestro-painel?erro=acesso_negado');
  } catch (e) {
