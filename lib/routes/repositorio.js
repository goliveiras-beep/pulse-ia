...
export default async function handler(req, res) {
  ...
  if(req.method==='POST') {
    ...
    try {
      ...
      // Compartilha a Raiz e as 6 categorias com o e-mail de cada colaborador ativo —
      // corrige o "Acesso negado" ao abrir documentos direto no Drive fora do app. Roda
      // de novo sempre que alguém novo entrar na equipe (não é automático ainda).
      if(action==='liberarAcesso') {
        ...
        const tarefas = PASTA_DEFS.flatMap(p => emails.map(email => ({ id: p.id, email })));
        const erros = [];
        // Em lotes pequenos com pausa entre eles — disparar tudo de uma vez batia no
        // limite de taxa da API do Drive (userRateLimitExceeded).
        const TAM_LOTE = 4;
        for (let i = 0; i < tarefas.length; i += TAM_LOTE) {
          const lote = tarefas.slice(i, i+TAM_LOTE);
          await Promise.all(lote.map(async t => {
            try { await driveShareUser(t.id, t.email, gestorToken); } catch(e) { erros.push(e.message); }
          }));
          if (i + TAM_LOTE < tarefas.length) await sleep(200);
        }
        return res.status(200).json({ok:true, total: tarefas.length, falhas: erros});
      }
