  ...
  if(req.method==='POST') {
    ...
    try {
      ...
      if(action==='liberarAcesso') {
        ...
        const erros = [];
        await Promise.all(PASTA_DEFS.flatMap(p => emails.map(async email => {
          try { await driveShareUser(p.id, email, gestorToken); } catch(e) { erros.push(e.message); }
        })));
        return res.status(200).json({ok:true, total: PASTA_DEFS.length*emails.length, falhas: erros});
      }
