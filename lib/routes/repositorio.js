...
// Lança erro detalhado em caso de falha — antes era "silenciosa" e o chamador nunca via
// que a API do Drive tinha rejeitado o compartilhamento.
async function driveShare(fileId, token, role='reader') {
  const r = await fetch(`https://www.googleapis.com/drive/v3/files/${fileId}/permissions?supportsAllDrives=true`, {
    method: 'POST',
    headers: { Authorization: 'Bearer '+token, 'Content-Type': 'application/json' },
    body: JSON.stringify({ role, type: 'anyone' }),
  });
  if (!r.ok) throw new Error(`${fileId}: ${r.status} ${await r.text()}`);
}

// Documento nativo do Google (Docs/Sheets/Slides) tem permissão própria, independente da
// pasta — compartilhar só a pasta não basta, precisa percorrer tudo e compartilhar cada
// arquivo individualmente. Limite de profundidade é só uma guarda contra loop infinito.
async function shareTreeRecursivo(folderId, token, depth=0, erros=[]) {
  try { await driveShare(folderId, token); } catch(e) { erros.push(e.message); }
  if (depth > 6) return erros;
  let items;
  try { items = await driveList(folderId, token); } catch(e) { erros.push(`listar ${folderId}: ${e.message}`); return erros; }
  await Promise.all(items.map(async item => {
    if (item.tipo === 'Pasta') { await shareTreeRecursivo(item.id, token, depth+1, erros); }
    else { try { await driveShare(item.id, token); } catch(e) { erros.push(e.message); } }
  }));
  return erros;
}
...
function renderPage({ session, arquivos, breadcrumb, currentId, isGestor, pastaDefs, ocultos, q }) {
  ...
  const rows = arquivos.length ? arquivos.map(a => {
    ...
    // Mover é liberado pra toda a equipe (mesmo critério do upload); renomear/excluir/
    // ocultar continuam só pra gestor.
    const btnMover = `<button onclick="moverItem('${esc(a.id)}','${esc(a.nome).replace(/'/g,"\\'")}')" title="Mover para outra pasta" style="background:none;border:1px solid var(--border);border-radius:5px;padding:3px 7px;font-size:11px;color:var(--muted);cursor:pointer">📦</button>`;
    const gestoAcoes = isGestor ? `
      ...
      </div>` : `<div class="acoes-col">${btnMover}</div>`;
...
async function liberarAcesso(){
  showToast('🔓 Liberando acesso...',true);
  try{
    var r=await fetch('/api/repositorio?action=liberarAcesso',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});
    var d=await r.json();
    if(d.ok&&(!d.falhas||!d.falhas.length))showToast('✅ Acesso liberado pra toda a equipe!',true);
    else if(d.ok)showToast('⚠️ '+d.falhas.length+' falha(s) — ver console',false);
    else showToast('Erro: '+(d.error||'?'),false);
    if(d.falhas&&d.falhas.length)console.error('liberarAcesso falhas:',d.falhas);
  }catch(e){showToast('Erro de conexão',false);}
}
  ...
  if(req.method==='POST') {
    ...
    if(action==='upload') {
      try {
        ...
        // Sem isso, quem abrir o link do arquivo fora do app cai em "Acesso negado"
        await driveShare(f.id, gestorToken);
    ...
    try {
      ...
      // Compartilha a Raiz e as 6 categorias com "qualquer pessoa com o link" — corrige
      // de uma vez o "Acesso negado" ao abrir documentos direto no Drive fora do app, e
      // cobre automaticamente qualquer pessoa nova que entrar na equipe depois.
      if(action==='liberarAcesso') {
        const gestorToken = await getGestorToken();
        const erros = await shareTreeRecursivo(REPO_ROOT, gestorToken);
        return res.status(200).json({ok:true, falhas: erros});
      }
