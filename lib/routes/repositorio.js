...
function renderPage({ session, arquivos, breadcrumb, currentId, isGestor, pastaDefs, ocultos, q }) {
  ...
  const rows = arquivos.length ? arquivos.map(a => {
    ...
    // Mover/excluir são liberados pra toda a equipe (mesmo critério do upload);
    // renomear/ocultar continuam só pra gestor.
    ...
    const btnExcluir = `<button onclick="excluirItem('${esc(a.id)}','${esc(a.nome).replace(/'/g,"\\'")}',${isFolder})" title="Excluir" style="background:none;border:1px solid #dc2626;border-radius:5px;padding:3px 7px;font-size:11px;color:#dc2626;cursor:pointer">🗑️</button>`;
    const gestoAcoes = isGestor ? `
      <div class="acoes-col">
        ...
        ${btnExcluir}
      </div>` : `<div class="acoes-col">${btnMover}${btnExcluir}</div>`;
