...
async function driveShare(fileId, token, role='reader') {
  ...
  } catch (e) { console.warn('driveShare falhou pra', fileId, e.message); }
}
...
async function shareTreeRecursivo(folderId, token, depth=0) {
  await driveShare(folderId, token);
  if (depth > 6) return;
  let items;
  try { items = await driveList(folderId, token); } catch { return; }
  await Promise.all(items.map(async item => {
    if (item.tipo === 'Pasta') await shareTreeRecursivo(item.id, token, depth+1);
    else await driveShare(item.id, token);
  }));
}
...
async function liberarAcesso(){
  ...
  try{
    ...
    var d=await r.json();
    if(d.ok)showToast('✅ Acesso liberado pra toda a equipe!',true);
    else showToast('Erro: '+(d.error||'?'),false);
  }catch(e){showToast('Erro de conexão',false);}
}
  ...
  if(req.method==='POST') {
    ...
    try {
      ...
      if(action==='liberarAcesso') {
        const gestorToken = await getGestorToken();
        await shareTreeRecursivo(REPO_ROOT, gestorToken);
        return res.status(200).json({ok:true});
      }
