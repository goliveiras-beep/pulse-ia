...
import { createSign, createHash, timingSafeEqual } from 'crypto';
...
// Cacheado em memória por instância (mesmo padrão que sheetsRequest já usa via
// google-auth.js) — sem isso, TODA requisição (inclusive so pra listar uma pasta)
// pagava um round-trip inteiro de assinatura JWT + troca de token com o Google antes
// de sequer chamar a Drive API, o que sozinho já explicava vários segundos de
// lentidão na Central de Conhecimento a cada clique.
const _cacheTokenDrive = {};
async function getDriveToken(readOnly=false) {
  const key = readOnly ? 'readonly' : 'full';
  const cached = _cacheTokenDrive[key];
  ...
  _cacheTokenDrive[key] = { token: d.access_token, exp: Date.now() + (d.expires_in||3600)*1000 };
...
let _cacheTokenGestor = null; // {token, exp} — mesma ideia do cache de getDriveToken acima
async function getGestorToken() {
  if(_cacheTokenGestor && _cacheTokenGestor.exp > Date.now()+60_000) return _cacheTokenGestor.token;
  ...
  _cacheTokenGestor = { token: d.access_token, exp: Date.now() + (d.expires_in||3600)*1000 };
  return d.access_token;
}
...
export default async function handler(req, res) {
  ...
  // Verificar perfil — as duas leituras de planilha (equipe + config de pastas ocultas)
  // não dependem uma da outra, então rodam em paralelo em vez de em série (isso sozinho
  // já cortava boa parte do tempo de carregar a tela, ver cache de token acima também).
  let isGestor = false;
  const [eq, ocultos] = await Promise.all([
    sheetsRequest(SHEET_ID,'/values/Equipe!A2:I200').then(d=>d.values||[]).catch(()=>[]),
    lerOcultos(),
  ]);
