...
function renderDia(dia) {
  ...
  // Encerrado continua na coluna "Próximos eventos", em ordem cronologica junto com o resto
  // (cor normal, sem apagar) - so nao entra no numero do badge, ja que nao e mais "proximo".
  const restoComEncerrados = [...resto, ...encerrados].sort((a, b) => (a.hora || '').localeCompare(b.hora || ''));
  ...
  return `
    <div class="grade-dia-cols" style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
      <div>
        <div style="font-size:11px;font-weight:700;color:var(--text3);margin-bottom:8px;display:flex;align-items:center;gap:6px">🕐 Eventos <span id="colProximosCount" class="badge" style="background:var(--blue-m-bg);color:var(--blue-m-v)">${resto.length}</span></div>
        <div id="colProximos">${restoComEncerrados.map(htmlDe).join('')}</div>
        <div id="colProximosEmpty" class="empty" style="display:${restoComEncerrados.length ? 'none' : 'block'}">Nenhum evento</div>
      </div>
      <div>
  ...
  return `
    ...
    <script>
      ...
      function reordenarEventosBooking() {
        ...
        var vivos = [], resto = []; // "resto" ja inclui os encerrados - ficam juntos, na mesma cor, so por horario
        var totalNaoEncerrados = 0;
        todos.forEach(function (c) {
          if (c.classList.contains('g-ev-encerrado')) { resto.push(c); return; }
          var promovido = c.classList.contains('g-ev-aovivo') || c.classList.contains('g-ev-p30') || c.classList.contains('g-ev-p60');
          if (promovido) { vivos.push(c); } else { resto.push(c); totalNaoEncerrados++; }
        });
        ...
        var pc = document.getElementById('colProximosCount'); if (pc) pc.textContent = totalNaoEncerrados;
        var ve = document.getElementById('colAoVivoEmpty'); if (ve) ve.style.display = vivos.length ? 'none' : 'block';
        var pe = document.getElementById('colProximosEmpty'); if (pe) pe.style.display = resto.length ? 'none' : 'block';
      }
      atualizarBadgesBookingDia();
