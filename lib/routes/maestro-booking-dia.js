...
// Mesma regra de "ao vivo" usada nas flags (ver atualizarBadgesBookingDia no script) - reaplicada
// aqui no servidor so pra decidir a ORDEM inicial da lista (evento ao vivo sobe pro topo, o resto
// segue por horario). Sem isso a pagina nasceria com a ordem errada ate o JS do cliente rodar.
function estaAoVivoAgora(hora, horaFim) {
  const agora = getBRT();
  ...
  if (f !== null) return min >= m && min <= f;
  return m <= min + 5 && m >= min - 30;
}
...
function renderDia(dia) {
  ...
  // "Ao vivo" (o que esta rolando agora) e "Próximos eventos" (o resto, por horario). O
  // evento muda de coluna sozinho quando comeca/termina (ver reordenarEventosBooking no
  // script, que reusa a mesma regra estaAoVivoAgora do lado do cliente).
  const eventos = [...dia.eventos].sort((a, b) => (a.hora || '').localeCompare(b.hora || ''));
  const idxPorEvento = new Map(eventos.map((ev, i) => [ev, i]));
  const aoVivo = eventos.filter((ev) => estaAoVivoAgora(ev.hora, ev.horaFim));
  const resto = eventos.filter((ev) => !estaAoVivoAgora(ev.hora, ev.horaFim));
    ...
    <script>
      ...
      function atualizarBadgesBookingDia() {
        ...
        document.querySelectorAll('[id^="eventoCard"][data-hora]').forEach(function (el) {
          ...
          if (el.dataset.isoje !== '1') { badge.innerHTML = ''; el.classList.remove('g-ev-aovivo', 'g-ev-p30', 'g-ev-p60'); return; }
          var m = toMinDiaBooking(el.dataset.hora); if (m === null) { badge.innerHTML = ''; el.classList.remove('g-ev-aovivo', 'g-ev-p30', 'g-ev-p60'); return; }
          var f = toMinDiaBooking(el.dataset.horafim || '');
          if (f !== null && f < m) f += 1440; // evento atravessa meia-noite
          var s = '';
          if (f !== null) {
            if (min >= m && min <= f) s = 'aovivo';
            else if (m - min > 0 && m - min <= 30) s = 'p30';
            else if (m - min > 0 && m - min <= 60) s = 'p60';
          } else {
            if (m <= min + 5 && m >= min - 30) s = 'aovivo';
            else if (m - min > 0 && m - min <= 30) s = 'p30';
            else if (m - min > 0 && m - min <= 60) s = 'p60';
          }
          if (s === 'aovivo') { badge.innerHTML = '<span style="background:#166534;color:#86efac;border-radius:4px;padding:1px 7px;font-size:10px;font-weight:700">● AO VIVO</span>'; el.classList.add('g-ev-aovivo'); el.classList.remove('g-ev-p30', 'g-ev-p60'); }
          else if (s === 'p30') { badge.innerHTML = '<span style="background:#451a03;color:#fcd34d;border-radius:4px;padding:1px 7px;font-size:10px;font-weight:700">⚡ &lt;30min</span>'; el.classList.add('g-ev-p30'); el.classList.remove('g-ev-aovivo', 'g-ev-p60'); }
          else if (s === 'p60') { badge.innerHTML = '<span style="background:#431407;color:#fb923c;border-radius:4px;padding:1px 7px;font-size:10px;font-weight:700">🔜 &lt;60min</span>'; el.classList.add('g-ev-p60'); el.classList.remove('g-ev-aovivo', 'g-ev-p30'); }
          else { badge.innerHTML = ''; el.classList.remove('g-ev-aovivo', 'g-ev-p30', 'g-ev-p60'); }
        });
        reordenarEventosBooking();
      }
      // Evento ao vivo muda de coluna sozinho (Ao vivo <-> Próximos eventos) assim que
      // comeca/termina, sem precisar recarregar a pagina - so existe quando o dia mostrado
      // e hoje (colunas so sao renderizadas nesse caso, ver renderDia no servidor).
      function reordenarEventosBooking() {
        ...
        var vivos = [], resto = [];
        todos.forEach(function (c) { (c.classList.contains('g-ev-aovivo') ? vivos : resto).push(c); });
