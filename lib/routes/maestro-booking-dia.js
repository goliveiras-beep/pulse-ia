...
const ICONE_MEIO = { 'Satélite': '📡', SRT: '🌐', Fibra: '🔌', LiveU: '📶', Decoder: '🖥️' };
...
function renderEvento(ev, idEv, isHoje) {
  ...
  return `
    <div style="border:1px solid var(--border);border-radius:8px;margin-bottom:10px;overflow:hidden">
      <div class="evento-header" data-hora="${esc(ev.hora || '')}" data-horafim="${esc(ev.horaFim || '')}" data-isoje="${isHoje ? 1 : 0}" onclick="toggleEventoBooking(${idEv})" style="background:var(--bg3);padding:8px 12px;display:flex;align-items:center;gap:10px">
  ...
  return `
    ...
    <script>
      ...
      function atualizarBadgesBookingDia() {
        ...
        var min = bh * 60 + bm;
        document.querySelectorAll('.evento-header[data-hora]').forEach(function (el) {
          var badge = el.querySelector('.badge-realtime-dia'); if (!badge) return;
          if (el.dataset.isoje !== '1') { badge.innerHTML = ''; return; }
          var m = toMinDiaBooking(el.dataset.hora); if (m === null) { badge.innerHTML = ''; return; }
