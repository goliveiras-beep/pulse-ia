...
function renderDia(dia) {
  ...
  const eventos = isHoje
    ? [...dia.eventos].sort((a, b) => {
        const aoVivoA = estaAoVivoAgora(a.hora, a.horaFim);
        const aoVivoB = estaAoVivoAgora(b.hora, b.horaFim);
        if (aoVivoA !== aoVivoB) return aoVivoA ? -1 : 1;
        return (a.hora || '').localeCompare(b.hora || '');
      })
    : dia.eventos;
  return eventos.map((ev, i) => renderEvento(ev, i, isHoje)).join('');
}
...
function renderPagina(diaInicial) {
  return `
    ...
    <script>
      ...
      // Evento ao vivo sobe pro topo da lista, mesmo que outros comecem antes dele - assim
      // que termina (classe g-ev-aovivo sai, ver acima), volta pra ordem normal por horario.
      function reordenarEventosBooking() {
        var body = document.getElementById('diaBookingBody');
        if (!body) return;
        var cards = Array.prototype.slice.call(body.querySelectorAll(':scope > [id^="eventoCard"]'));
        if (cards.length < 2) return;
        cards.sort(function (a, b) {
          var aoVivoA = a.classList.contains('g-ev-aovivo');
          var aoVivoB = b.classList.contains('g-ev-aovivo');
          if (aoVivoA !== aoVivoB) return aoVivoA ? -1 : 1;
