...
// Cada linha comeca fechada, so com o resumo (hora/titulo/tags) - clicar em qualquer parte
// do cabecalho expande os detalhes tecnicos (ficha, suporte, anexo, observacao), com o
// chevron girando e a altura animada via grid-template-rows (ver .evento-body em
// lib/maestro-layout.js). Por padrao so uma linha aberta por vez (ver toggleEventoBooking).
function renderEvento(ev, idEv, isHoje, opaco) {
  ...
  return `
    <div id="eventoCard${idEv}" class="${opaco ? 'g-ev-encerrado' : ''}" data-hora="${esc(ev.hora || '')}" data-horafim="${esc(ev.horaFim || '')}" data-isoje="${isHoje ? 1 : 0}" style="border:1px solid var(--border);border-radius:8px;margin-bottom:10px;overflow:hidden${opaco ? ';opacity:.4' : ''}">
      <div class="evento-header" onclick="toggleEventoBooking(${idEv})" style="background:var(--bg3);padding:8px 12px;display:flex;align-items:center;gap:10px;flex-wrap:wrap">
        ...
        <span class="evento-chevron" id="eventoChevron${idEv}">&#9662;</span>
      </div>
      <div class="evento-body" id="eventoBody${idEv}"><div>
...
function renderDia(dia) {
  ...
  // So pro dia de hoje: divide em duas colunas, igual a Home (#NossoDia/#NossoDiaAmanhã) -
  // "Próximos eventos" (mais de 1h pra comecar) e "Ao vivo" (rolando agora OU comecando em
  // ate 1h - "promovido" pra chamar atencao antes do horario). Evento ja encerrado nao entra
  // em nenhuma das duas (nao conta mais como "proximo"). Muda de coluna/some sozinho quando
  // comeca/termina (ver reordenarEventosBooking no script, mesma regra estadoEvento).
  ...
  const resto = eventos.filter((ev) => estadoEvento(ev.hora, ev.horaFim) === 'proximo');
  const encerrados = eventos.filter((ev) => estadoEvento(ev.hora, ev.horaFim) === 'encerrado');
  const htmlDe = (ev) => renderEvento(ev, idxPorEvento.get(ev), true);
  // Encerrado continua aparecendo (so mais apagado, pra referencia) mas nao conta no numero
  // de "Próximos eventos" - ele ja passou, nao e mais um proximo evento de verdade.
  const htmlDeEncerrado = (ev) => renderEvento(ev, idxPorEvento.get(ev), true, true);

  return `
    <div class="grade-dia-cols" style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
      <div>
        <div style="font-size:11px;font-weight:700;color:var(--text3);margin-bottom:8px;display:flex;align-items:center;gap:6px">🕐 Próximos eventos <span id="colProximosCount" class="badge" style="background:var(--blue-m-bg);color:var(--blue-m-v)">${resto.length}</span></div>
        <div id="colProximos">${resto.map(htmlDe).join('')}${encerrados.map(htmlDeEncerrado).join('')}</div>
        <div id="colProximosEmpty" class="empty" style="display:${resto.length || encerrados.length ? 'none' : 'block'}">Nenhum evento</div>
      </div>
      <div>
  ...
  return `
    ...
    <script>
      ...
      async function irParaDiaBooking(novaData) {
        ...
        _eventoBookingAberto = null; // trocar de dia sempre comeca com tudo fechado
      ...
      // Accordion dos eventos - por padrao so uma linha aberta por vez (MULTIPLOS=false).
      // Pra permitir varias abertas ao mesmo tempo no futuro, so trocar essa constante.
      var MULTIPLOS_EVENTOS_ABERTOS = false;
      var _eventoBookingAberto = null;
      function toggleEventoBooking(idEv) {
        var corpo = document.getElementById('eventoBody' + idEv);
        var chevron = document.getElementById('eventoChevron' + idEv);
        if (!corpo || !chevron) return;
        var vaiAbrir = !corpo.classList.contains('open');
        if (!MULTIPLOS_EVENTOS_ABERTOS && _eventoBookingAberto !== null && _eventoBookingAberto !== idEv) {
          var corpoAnterior = document.getElementById('eventoBody' + _eventoBookingAberto);
          var chevronAnterior = document.getElementById('eventoChevron' + _eventoBookingAberto);
          if (corpoAnterior) corpoAnterior.classList.remove('open');
          if (chevronAnterior) chevronAnterior.classList.remove('open');
        }
        corpo.classList.toggle('open', vaiAbrir);
        chevron.classList.toggle('open', vaiAbrir);
        _eventoBookingAberto = vaiAbrir ? idEv : null;
      }
      ...
      // Evento sobe pra "Ao vivo" sozinho (rolando agora OU comecando em ate 1h), volta pra
      // "Próximos eventos" ou some da tela (ja encerrado) - sem precisar recarregar a pagina.
      // So existe quando o dia mostrado e hoje (colunas so sao renderizadas nesse caso).
      function reordenarEventosBooking() {
        var colVivo = document.getElementById('colAoVivo');
        var colProx = document.getElementById('colProximos');
        if (!colVivo || !colProx) return; // dia != hoje, sem colunas - nada a fazer
        var todos = Array.prototype.slice.call(document.querySelectorAll('[id^="eventoCard"]'));
        var vivos = [], resto = [], encerrados = [];
        todos.forEach(function (c) {
          if (c.classList.contains('g-ev-encerrado')) { encerrados.push(c); return; } // ja passou - fica no fim, mais apagado (nao conta)
          var promovido = c.classList.contains('g-ev-aovivo') || c.classList.contains('g-ev-p30') || c.classList.contains('g-ev-p60');
          (promovido ? vivos : resto).push(c);
        });
        vivos.sort(function (a, b) { return (a.dataset.hora || '').localeCompare(b.dataset.hora || ''); });
        resto.sort(function (a, b) { return (a.dataset.hora || '').localeCompare(b.dataset.hora || ''); });
        encerrados.sort(function (a, b) { return (a.dataset.hora || '').localeCompare(b.dataset.hora || ''); });
        vivos.forEach(function (c) { colVivo.appendChild(c); });
        resto.forEach(function (c) { colProx.appendChild(c); });
        encerrados.forEach(function (c) { colProx.appendChild(c); });
        var vc = document.getElementById('colAoVivoCount'); if (vc) vc.textContent = vivos.length;
        var pc = document.getElementById('colProximosCount'); if (pc) pc.textContent = resto.length;
        var ve = document.getElementById('colAoVivoEmpty'); if (ve) ve.style.display = vivos.length ? 'none' : 'block';
        var pe = document.getElementById('colProximosEmpty'); if (pe) pe.style.display = (resto.length || encerrados.length) ? 'none' : 'block';
      }
      atualizarBadgesBookingDia();
