...
async function sincronizarTodasAgendas(horizonteStr) {
  ...
  // Coluna N = refresh token de Calendar (coluna M já é tipoContrato — ver equipe-view.js/banco-horas.js)
  const comAgenda = equipeRaw.filter(r => r[0] && (r[10]||'ativo').toLowerCase() === 'ativo' && r[13]);

  const resultados = await Promise.all(comAgenda.map(async p => {
    const nome = p[0], refreshToken = p[13];
