...
export default async function handler(req, res) {
  ...
  if (usuario) {
    ...
    if (status === 'ativo') {
      ...
      // Persiste o refresh token (com scope de Calendar, pedido no login) na coluna M —
      ...
      if (refreshToken) {
        sheetsRequest(process.env.GOOGLE_SHEET_ID, `/values/${encodeURIComponent('Equipe!M' + (usuarioIdx + 2))}?valueInputOption=USER_ENTERED`, 'PUT', { values: [[refreshToken]] }).catch(() => {});
      }
