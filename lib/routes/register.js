...
export default async function handler(req, res) {
  ...
  const usuarioIdx = equipe.findIndex(r => (r[9] || '').toLowerCase() === email.toLowerCase());
  const usuario = usuarioIdx >= 0 ? equipe[usuarioIdx] : null;

  if (usuario) {
    const status = (usuario[10] || 'ativo').toLowerCase();
    if (status === 'ativo') {
      setSession(res, usuario[0], accessToken, refreshToken);
      // Persiste o refresh token (com scope de Calendar, pedido no login) na coluna N —
      // (coluna M já é usada pra tipoContrato, ver equipe-view.js/banco-horas.js — não reusar)
      // fica disponível pra sincronizar a agenda mesmo fora de uma sessão ativa (ex: quando
      // o gestor publica o horizonte). Só grava se veio um refresh token novo (nem sempre vem).
      if (refreshToken) {
        sheetsRequest(process.env.GOOGLE_SHEET_ID, `/values/${encodeURIComponent('Equipe!N' + (usuarioIdx + 2))}?valueInputOption=USER_ENTERED`, 'PUT', { values: [[refreshToken]] }).catch(() => {});
      }
