  ...
  if(colaboradores.length===0){
    ...
    const linhasHtml=colaboradores.map((nome,i)=>{
      ...
      return periodos.map(a=>{
        ...
        // Barra estreita não cabe "DD/MM → DD/MM" sem cortar — mostra só o ícone nesse caso
        // (a data completa continua disponível no title ao passar o mouse ou no clique).
        const label=width>=7?`${ic} ${a.ini} → ${a.fim||a.ini}`:ic;
