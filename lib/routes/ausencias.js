...
function getSession(req){
  ...
  try{
    ...
    if(!assinaturaBate(h, hash(data + ts))||data.startsWith('~~OAUTH~~'))return null;
...
// Datas na planilha são só DD/MM (sem ano) — pra ordenar por ordem real de acontecimento (não
// por ordem de aprovação/cadastro), escolhe entre ano-1/ano/ano+1 o que cai mais perto de "hoje".
// Sem isso, uma folga marcada pra janeiro do ano que vem (comum em época de férias) é lida como
// já tendo acontecido em janeiro deste ano, e a lista de ausências sai fora de ordem cronológica.
function dfParaDateProxima(df,hoje){
  const[d,m]=df.split('/').map(Number);
  const ano=hoje.getFullYear();
  const candidatos=[ano-1,ano,ano+1].map(a=>new Date(a,m-1,d));
  return candidatos.reduce((mais,c)=>Math.abs(c-hoje)<Math.abs(mais-hoje)?c:mais);
}
// Data de fim "real" de uma ausência (início já desambiguado + fim no mesmo ano do início, ou no
// ano seguinte se a ausência virar o ano). Usada pra saber se já foi concluída.
function fimDataAbsoluta(a,hoje){
  const iniD=dfParaDateProxima(a.ini,hoje);
  const fimStr=a.fim||a.ini;
  const fimMes=Number(fimStr.split('/')[1]);
  const fimAno=fimMes<iniD.getMonth()+1?iniD.getFullYear()+1:iniD.getFullYear();
  return dfParaDate(fimStr,fimAno);
}
...
export default async function handler(req,res){
  ...
  const porData=lista=>[...lista].sort((a,b)=>dfParaDateProxima(a.ini,hoje)-dfParaDateProxima(b.ini,hoje));

  // Ausência aprovada cujo fim já passou vira histórico sozinha — some da lista de Aprovadas e
  // da timeline, e só aparece se o gestor for procurar na aba Histórico.
  const hojeSemHora=new Date(hoje.getFullYear(),hoje.getMonth(),hoje.getDate());
  const jaConcluida=a=>fimDataAbsoluta(a,hoje)<hojeSemHora;

  const pendentes=porData(ausencias.filter(a=>a.status==='pendente'));
  const aprovadas=porData(ausencias.filter(a=>a.status==='aprovado'&&!jaConcluida(a)));
  const historico=porData(ausencias.filter(a=>a.status==='recusado'||a.status==='cancelado'||(a.status==='aprovado'&&jaConcluida(a))));
  ...
  if(colaboradores.length===0){
    ...
    const comDatas=aprovadas.map(a=>({...a,iniD:dfParaDateProxima(a.ini,hoje),fimD:fimDataAbsoluta(a,hoje)}));
    ...
    const linhasHtml=colaboradores.map((nome,i)=>{
      ...
      return periodos.map(a=>{
        ...
        const numDias=Math.round((a.fimD-a.iniD)/DIA_MS)+1;
        const texto=numDias>1?`${a.ini} → ${a.fim||a.ini}`:a.ini;
        // Toda barra mostra a data — sempre no mesmo padrão. Quando cabe dentro do bloco
        // colorido, fica centralizada; quando a barra é curta demais, a data "vaza" pra fora
        // como uma etiqueta anexada à direita, em vez de sumir (padrão comum de Gantt chart).
        const dentro=width>=8;
        const spanStyle=dentro
          ?`font-size:9px;font-weight:800;color:${c};white-space:nowrap`
          :`font-size:9px;font-weight:700;color:var(--text2);white-space:nowrap;position:absolute;top:50%;left:100%;transform:translateY(-50%);margin-left:6px;background:var(--card);border:1px solid var(--border);border-radius:4px;padding:1px 6px;box-shadow:0 1px 3px rgba(0,0,0,.15)`;
        return `<div class="aus-bar" onclick="abrirEditarAusencia(this)" data-id="${escAttr(a.id)}" data-nome="${escAttr(a.nome)}" data-tipo="${escAttr(a.tipo)}" data-ini="${escAttr(a.ini)}" data-fim="${escAttr(a.fim||a.ini)}" data-motivo="${escAttr(a.motivo||'')}" style="position:absolute;left:${left}%;width:${width}%;top:${i*ROW_H+3}px;height:${ROW_H-10}px;border-radius:8px;background:${bg};border:1px solid ${c};border-left:3px solid ${c};box-shadow:0 1px 3px rgba(0,0,0,.25);display:flex;align-items:center;justify-content:center;z-index:1;cursor:pointer" title="${a.tipo}: ${a.ini} → ${a.fim||a.ini} (${numDias} dia${numDias>1?'s':''}) — clique pra editar">
          <span style="${spanStyle}">${texto}</span>
        </div>`;
      }).join('');
    }).join('');
  ...
  // Extrai o nome do colega do texto do motivo ("Troca com Fulano: meu dia...") — é onde
  // essa informação está guardada pra Troca de horário, não tem coluna própria pra isso.
  function colegaDaTroca(motivo) {
    const m = String(motivo||'').match(/Troca com ([^:]+):/);
    return m ? m[1].trim() : '';
  }
    ...
    return lista.map(a=>{
      ...
      return `<div style="background:var(--card);border:1px solid ${comAcoes?c:'var(--border)'};border-radius:10px;padding:14px 16px;display:flex;align-items:center;gap:14px;margin-bottom:8px">
        ...
        <div style="flex-shrink:0;display:flex;flex-direction:column;gap:6px;align-items:flex-end">
          ${comAcoes&&a.tipo!=='Troca de horário'?`
          ...
          `:comAcoes&&a.tipo==='Troca de horário'?`<span style="font-size:10px;color:#718096;font-weight:600;text-align:right;max-width:130px">Aguardando ${colegaDaTroca(a.motivo)||'o colega'} aceitar — gestor não aprova</span>`:podeGerenciar?botoesGerenciar(a):`<span style="font-size:10px;color:${a.status==='aprovado'?'#4ade80':a.status==='recusado'?'#fc8181':'#718096'};font-weight:600;text-transform:uppercase">${a.status==='aprovado'?'✓ Aprovado':a.status==='recusado'?'✕ Recusado':'Cancelado'}</span>`}
        </div>
      </div>`;
    }).join('');
  }
  ...
  function renderTabela(lista, comAcoes=false, podeGerenciar=false){
    ...
    const linhas=lista.map(a=>{
      ...
      return `<tr style="border-bottom:1px solid var(--border)">
        ...
        <td style="padding:8px">${comAcoes&&a.tipo!=='Troca de horário'?`
          ...
          </div>`:comAcoes&&a.tipo==='Troca de horário'?`<span style="font-size:10px;color:#718096;font-weight:600">Aguardando ${colegaDaTroca(a.motivo)||'colega'}</span>`:podeGerenciar?`<div style="display:flex;gap:6px">${botoesGerenciar(a,true)}</div>`:`<span style="font-size:10px;font-weight:600;text-transform:uppercase;color:${a.status==='aprovado'?'#4ade80':a.status==='recusado'?'#fc8181':'var(--text3)'}">${a.status==='aprovado'?'✓ Aprovado':a.status==='recusado'?'✕ Recusado':'Cancelado'}</span>`}</td>
      </tr>`;
    }).join('');
