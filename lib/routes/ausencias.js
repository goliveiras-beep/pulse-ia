...
export default async function handler(req,res){
  ...
  function badgeTipo(tipo,dark=true){
    ...
    return `<span style="background:${c[0]};color:${c[1]};border-radius:5px;padding:2px 8px;font-size:10px;font-weight:700">${c[2]} ${label}</span>`;
  }
  ...
  if(colaboradores.length===0){
    ...
    const AVISO_ROW_H=15;
    const AVISOS_H=blocos.length?blocos.length*AVISO_ROW_H+3:0;
    const avisosHtml=blocos.map((b,bi)=>{
      const iniD=new Date(b.iniKey*DIA_MS);
      const fimD=new Date(b.fimKey*DIA_MS);
      const left=pct(iniD);
      let maxAusentes=0;
      for(let k=b.iniKey;k<=b.fimKey;k++)maxAusentes=Math.max(maxAusentes,diaParaNomes.get(k).size);
      return `<div style="position:absolute;left:${left}%;top:${bi*AVISO_ROW_H}px;font-size:9px;font-weight:800;color:#fb923c;background:rgba(251,146,60,.14);border-radius:4px;padding:1px 6px;white-space:nowrap;z-index:2">⚠ ${fmtData(iniD)}→${fmtData(fimD)} · ${maxAusentes} ausentes</div>`;
    }).join('');
    ...
    const detalheDiasHtml=blocos.map(b=>{
      ...
      for(let k=b.iniKey;k<=b.fimKey;k++){
        ...
        dias.push(`<div>
          <div style="font-size:11px;font-weight:700;color:var(--text)">${DIAS_ABR[d.getDay()]} · ${fmtData(d)}</div>
          <div style="font-size:10px;color:var(--text2)">${[...diaParaNomes.get(k)].join(', ')}</div>
        </div>`);
      }
      return `<div style="background:var(--bg);border:1px solid #fb923c;border-radius:10px;padding:10px 14px;margin-bottom:10px">
    ...
    const nomesHtml=colaboradores.map(nome=>{
      const periodos=comDatas.filter(a=>a.nome===nome);
      return `<div style="height:${ROW_H}px;display:flex;align-items:center;gap:8px;border-bottom:1px solid var(--border)">
...
<div style="max-width:1100px;margin:0 auto;padding:20px 16px">
  ...
  <!-- Timeline -->
  <div style="background:var(--card);border:1px solid var(--border);border-radius:12px;padding:16px;margin-bottom:24px">
