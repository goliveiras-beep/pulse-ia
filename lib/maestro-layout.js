...
.evento-modal-bg{display:none;position:fixed;inset:0;background:rgba(0,0,0,.55);z-index:300;align-items:center;justify-content:center;padding:16px}
.evento-modal-bg.open{display:flex}
.evento-modal{background:var(--card);border-radius:12px;width:100%;max-width:560px;max-height:85vh;overflow-y:auto;box-shadow:0 20px 60px rgba(0,0,0,.45)}
...
.m-keep{display:flex}
/* Mobile: mesmo padrao da Home (api/app.js) - esconde tudo no .hr exceto o que tem
   .m-keep (relogio, tema, menu), pra nao estourar a largura da tela com o widget de
   clima/nome/"Voltar ao Pulse" todos numa linha so. */
@media(max-width:640px){
  ...
  #mrelogio-brt{font-size:13px!important;font-weight:700!important}
  .wrap table{display:block;overflow-x:auto;-webkit-overflow-scrolling:touch}
  .grade-dia-cols{grid-template-columns:1fr!important}
}
</style>
</head>
<body>
<div class="header">
  ...
  <div class="hr">
    ...
    <div class="m-keep" style="flex-direction:column;align-items:flex-end;gap:1px">
    ...
    <button id="mtt" class="btn-sm m-keep" onclick="toggleTheme()">🌙/☀️</button>
    <div class="m-keep" style="position:relative">
      <button id="mmenu-btn" onclick="toggleMenuMaestro(event)" aria-label="Menu" class="btn-sm" style="font-size:15px;padding:4px 10px;line-height:1">☰</button>
      <div id="mmenu-dropdown" style="display:none;position:absolute;top:calc(100% + 8px);right:0;background:var(--card);border:1px solid var(--border);border-radius:8px;box-shadow:0 8px 24px rgba(0,0,0,.35);min-width:210px;overflow:hidden;z-index:200">
...
function toggleMenuMaestro(e){
  if(e) e.stopPropagation();
  var d=document.getElementById('mmenu-dropdown');
  d.style.display = d.style.display==='block' ? 'none' : 'block';
}
document.addEventListener('click', function(e){
  var d=document.getElementById('mmenu-dropdown'), btn=document.getElementById('mmenu-btn');
  if(d && d.style.display==='block' && !d.contains(e.target) && e.target!==btn) d.style.display='none';
});
