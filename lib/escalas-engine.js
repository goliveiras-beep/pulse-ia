...
export function analisarDia(turnoAtual, turnoAnterior, turnoProximo, ausencia, diasConsecutivos) {
  ...
  if (ausencia) {
    return { alertas: [], status: ausencia[3] || 'Ausência', tipo: 'ausencia' };
  }
...
export function calcularConsecutivos(escala, ausencias, nome, dataRef) {
  let consecutivos = 0;
  ...
  for (let i = 1; i <= 14; i++) {
    ...
    const turno = escala.find(r => r[2] === nome && r[0] === df);
    const aus = ausencias.find(a => a[1] === nome && statusAusencia(a[0]) === 'aprovado' && dentroPeriodo(a[4], a[5], df));
...
export function analisarEscala(escala, ausencias, nomes, datas) {
  ...
  nomes.forEach(nome => {
    ...
    datas.forEach((df, idx) => {
      const turnoAtual = escala.find(r => r[2] === nome && r[0] === df);
      ...
      const turnoAntRaw = dfAnterior ? escala.find(r => r[2] === nome && r[0] === dfAnterior) : null;
      ...
      const turnoProxRaw = dfProximo ? escala.find(r => r[2] === nome && r[0] === dfProximo) : null;
      ...
      const consec = calcularConsecutivos(escala, ausencias, nome, dataObj);
