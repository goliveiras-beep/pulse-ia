...
export async function debugCamposBookingDoDia(dataAirtableStr, valores) {
  const filtro = `AND({Status}!='Cancelado', DATESTR({Início do Evento BRT})='${dataAirtableStr}')`;
  const r = await fetch(
    `https://api.airtable.com/v0/${AIRTABLE_BASE}/${AIRTABLE_TABLE_BOOKING}?filterByFormula=${encodeURIComponent(filtro)}&maxRecords=5&cellFormat=string&timeZone=America%2FSao_Paulo&userLocale=pt-BR`,
    { headers: { Authorization: `Bearer ${process.env.AIRTABLE_PAT || process.env.AIRTABLE_API_KEY}` } }
  );
  const d = await r.json();
