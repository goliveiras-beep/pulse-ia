...
export async function debugCamposBookingDoDia(dataAirtableStr) {
  const filtro = `AND({Status}!='Cancelado', DATESTR({Início do Evento BRT})='${dataAirtableStr}')`;
  const r = await fetch(
    `https://api.airtable.com/v0/${AIRTABLE_BASE}/${AIRTABLE_TABLE_BOOKING}?filterByFormula=${encodeURIComponent(filtro)}&maxRecords=3`,
    { headers: { Authorization: `Bearer ${process.env.AIRTABLE_PAT || process.env.AIRTABLE_API_KEY}` } }
  );
  const d = await r.json();
