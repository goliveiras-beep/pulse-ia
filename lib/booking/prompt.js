...
- "EBU REF: 26-202262/1" → o "/1" também é versão, não faz parte do número:
  referenceNumber "26-202262", version 1. Mesma regra vale pra qualquer sufixo
  "/N" depois de um número de referência - N é a versão, sempre.
- referenceNumber é SEMPRE o número puro, sem a palavra "version" e sem sufixo
  "/N" nenhum. É a chave que casa um amendment com o documento original - se
  você incluir a versão nele (com espaço, barra, ou qualquer separador), a
  deduplicação quebra e a base ganha duplicatas.
- Coloque em secondaryRefs toda outra referência citada (SE, Event number,
  códigos do fornecedor) - MAS NÃO o EBU REF em si, que já é o referenceNumber
  quando referenceType="ebu_ref". Essas referências secundárias casam
  documentos irmãos do mesmo jogo.
...
## Formato de saída
Responda APENAS com um objeto JSON válido, usando EXATAMENTE estas chaves (nada de
inventar nome de campo diferente, nem aninhar diferente do que está aqui). Nenhum
texto, comentário ou markdown antes ou depois do JSON. Campo sem valor no documento
é null (ou [] se for lista) - nunca omita a chave.

{
  "event": {
    "sourceRef": {
      "provider": "vivaro" | "sportcast" | "eurovision" | "conmebol" | "outro",
      "referenceType": "synopsis" | "ebu_ref" | "service_order" | "event_number" | "outro",
    ...
    "transmissionStartUtc": "AAAA-MM-DDTHH:MM:SSZ" | null,
    "transmissionEndUtc": "AAAA-MM-DDTHH:MM:SSZ" | null,
    "kickoffUtc": "AAAA-MM-DDTHH:MM:SSZ" | null,
    "signalPaths": [
      {
        "label": string,
        "medium": "satellite" | "srt" | "udp" | "rtmp" | "fiber" | "outro",
        "role": "main" | "backup" | "unknown",
        "origin": string | null,
        "destination": string | null,
        "startUtc": string | null,
        "endUtc": string | null,
        "satellite": { "satelliteName": string|null, "orbitalPosition": string|null, "transponder": string|null, "channel": string|null, "channelBandwidthMhz": number|null, "serviceSlot": string|null, "serviceName": string|null, "serviceId": string|null, "uplinkFreqMhz": number|null, "uplinkPolarization": string|null, "downlinkFreqMhz": number|null, "downlinkPolarization": string|null, "modulation": string|null, "symbolRateMsps": number|null, "fec": string|null, "rollOff": number|null, "pilot": boolean|null, "carrierMode": string|null, "nlcMode": string|null } | null,
        "ip": { "protocol": string|null, "mode": "listener"|"caller"|"rendezvous"|"desconhecido"|null, "url": string|null, "host": string|null, "port": number|null, "minLatencyMs": number|null, "whitelistCidrs": string[], "resourceName": string|null } | null,
        "fiber": { "serviceDescription": string|null, "bandwidthMbps": number|null, "durationMinutes": number|null, "approxOutMinutes": number|null } | null,
        "video": { "standard": string|null, "resolution": string|null, "frameRate": string|null, "aspectRatio": string|null, "codec": string|null, "chromaSubsampling": string|null, "bitrateMbps": number|null, "dynamicRange": string|null } | null,
        "encryption": { "type": "biss1"|"biss2"|"hmcrypt"|"passphrase"|"clear"|"desconhecido", "key": string|null } | null,
        "audio": [ { "channel": number, "label": string } ],
        "notes": string | null
      }
    ]
  },
  "confidence": { "caminho.pontuado.do.campo": "high" | "medium" | "low" | "absent" },
  "warnings": [ string ]
}

Só inclua "satellite" quando medium="satellite" (os outros ficam null), só "ip" quando
medium for "srt"/"udp"/"rtmp", só "fiber" quando medium="fiber" - mas as TRÊS chaves
sempre aparecem no objeto do caminho, valendo null quando não se aplica.`;
