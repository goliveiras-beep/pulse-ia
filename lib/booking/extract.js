...
async function chamarGroq(messages) {
  // O 413 anterior NAO era o documento grande - era este max_tokens:8000 somado ao overhead
  // fixo de prompt+schema (~4.000 tokens) já passando da cota de 8.000 tokens/min do tier
  // "on_demand" da Groq (a API conta prompt + max_tokens pedido contra essa cota, nao só o
  // que o modelo de fato gera). Contas batem: 1573 (system) + 2430 (schema) + 8000
  // (max_tokens) = 12003, + ~880 do texto do documento = 12885, exatamente o "Requested" do
  // erro. Corrigido: max_tokens baixado pro tamanho real que uma extração precisa, e o
  // response_format tirado da chamada (era ~2.430 tokens de overhead por chamada) - a
  // validação Zod + autocorreção (ver extrairDocumento) já garante a forma da saída sem
  // precisar forçar isso do lado da API.
