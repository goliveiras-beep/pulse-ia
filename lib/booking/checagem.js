...
export async function checarPendenciasBooking(modo = 'manha') {
  ...
  await Promise.all(alertas.map(async (a) => {
    ...
    // arquiva cada e-mail achado (PDF + anexos na Central de Conhecimento) - so uma vez
    // por e-mail, nunca de novo nas proximas execucoes (ver registro-arquivamento.js)
