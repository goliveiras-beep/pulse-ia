...
export async function checarPendenciasBooking(modo = 'manha') {
  ...
  // limite por execucao - cada arquivamento faz varias chamadas de API (pasta + PDF +
  // upload + anexos); sem limite, muitos e-mails de uma vez passam do maxDuration da
  // function (aconteceu de verdade: 504). O que nao arquivar agora fica pra proxima
  // execucao (jaArquivado() pula o que ja foi feito, nunca reprocessa).
  const LIMITE_ARQUIVAMENTOS_POR_EXECUCAO = 6;
  let arquivadosNestaExecucao = 0;

  for (const a of alertas) {
    for (const email of a.emailsRelacionados) {
      if (await jaArquivado(email.id)) continue;
      if (arquivadosNestaExecucao >= LIMITE_ARQUIVAMENTOS_POR_EXECUCAO) {
