...
// TEMPORARIO - checar se "PGM (horario)" na tabela Booking tem data completa ou so a hora.
// Remover depois de usado uma vez.
export async function debugPgmBooking() {
  const filtro = `AND({Status}!='Cancelado', {Se Aplica Booking de Sinal?}='Sim', IS_AFTER({Início do Evento BRT}, '${new Date().toISOString()}'))`;
  const r = await fetch(
    `https://api.airtable.com/v0/${AIRTABLE_BASE}/${AIRTABLE_TABLE}?filterByFormula=${encodeURIComponent(filtro)}&maxRecords=5`,
    { headers: { Authorization: `Bearer ${process.env.AIRTABLE_PAT || process.env.AIRTABLE_API_KEY}` } }
  );
  const d = await r.json();
  return (d.records || []).map((rec) => ({
    nome: rec.fields['Match ID'],
    inicioEventoBrt: rec.fields['Início do Evento BRT'],
    pgmHorario: rec.fields['PGM (horário)'],
  }));
}
