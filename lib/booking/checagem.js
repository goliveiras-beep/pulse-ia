...
function avaliarEvento(record) {
  ...
  for (const meio of meios) {
    ...
    if (!temAlgum(f, campos)) {
      pendencias.push(`Meio "${meio}" declarado, mas nenhum parâmetro técnico preenchido (${campos.join(', ')}).`);
    }
  }

  if (!meios.length) {
    pendencias.push('Nenhum meio de recepção definido ainda (campo "Origem do Sinal" vazio).');
  }

  if (!pendencias.length) return { nome, prioridade: 'OK', pendencias: [] };
