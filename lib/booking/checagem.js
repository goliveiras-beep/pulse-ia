...
// TEMPORARIO - buscar bookings ja preenchidos de uma competicao pra usar como referencia de
// preset (pedido do Guilherme 2026-09-14). Remover depois de usado.
export async function debugBookingsPorCompeticao(nomeCompeticao) {
  const filtro = `SEARCH('${nomeCompeticao}', {aux. Competição tarefa}&'')`;
  const r = await fetch(
    `https://api.airtable.com/v0/${AIRTABLE_BASE}/${AIRTABLE_TABLE}?filterByFormula=${encodeURIComponent(filtro)}&maxRecords=10&cellFormat=string&timeZone=America%2FSao_Paulo&userLocale=pt-BR`,
...
// TEMPORARIO - testar se PATCH funciona na tabela Booking, escrevendo um valor de teste num
// campo vazio e devolvendo o resultado (quem chama decide se limpa de volta). Remover depois.
export async function debugTestarEscrita(recordId, campo, valor) {
  const r = await fetch(`https://api.airtable.com/v0/${AIRTABLE_BASE}/${AIRTABLE_TABLE}/${recordId}`, {
    method: 'PATCH',
    headers: { Authorization: `Bearer ${process.env.AIRTABLE_PAT || process.env.AIRTABLE_API_KEY}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({ fields: { [campo]: valor } }),
  });
