...
// TEMPORARIO - listar todos os campos de um registro da tabela Booking pra achar o nome
// exato do campo de horario "PGM" (se existir). Remover depois de usado uma vez.
export async function debugCamposBookingTodos() {
  const filtro = `AND({Status}!='Cancelado', {Se Aplica Booking de Sinal?}='Sim', IS_AFTER({Início do Evento BRT}, '${new Date().toISOString()}'))`;
  const r = await fetch(
    `https://api.airtable.com/v0/${AIRTABLE_BASE}/${AIRTABLE_TABLE}?filterByFormula=${encodeURIComponent(filtro)}&maxRecords=2&cellFormat=string&timeZone=America%2FSao_Paulo&userLocale=pt-BR`,
