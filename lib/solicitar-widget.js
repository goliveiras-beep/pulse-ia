...
const TIPO_CORES_BTN = {
  ...
  'Viagem': ['#cffafe','#0e7490'],
};
function badgeTipo(tipo) {
  const [bg, c] = TIPO_CORES_BTN[tipo] || ['#f3f4f6','#374151'];
  const label = tipo === 'Folga programada' ? 'Folga Programada' : tipo;
  return `<span style="background:${bg};color:${c};border-radius:4px;padding:2px 7px;font-size:10px;font-weight:600">${label}</span>`;
}
...
export async function solicitarBtn(nome) {
  // Equipe (13 col, só 0=nome e 10=status são usados aqui)
  ...
  return `
<style>
#sol-btn{transition:opacity .25s,transform .25s}
#sol-btn.sol-scrolling{opacity:.35;transform:scale(.7)!important;pointer-events:none}
@media(max-width:600px){#sol-btn{width:44px;height:44px;font-size:18px;bottom:16px;left:16px}}
</style>
  ...
  <div style="overflow-y:auto;padding:12px 14px;flex:1">
    <div id="sol-form-area">
      ...
      <div style="margin-bottom:12px">
        <label style="display:block;font-size:10px;font-weight:600;color:var(--text3);text-transform:uppercase;margin-bottom:4px">Motivo (opcional)</label>
...
function toggleSolicitar(){solAberto=!solAberto;var box=document.getElementById('sol-box');box.style.display=solAberto?'flex':'none';var btn=document.getElementById('sol-btn');btn.classList.remove('sol-scrolling');btn.style.transform=solAberto?'scale(0.9)':'scale(1)';}
var solScrollTimer=null;
window.addEventListener('scroll',function(){
  if(solAberto)return;
  var btn=document.getElementById('sol-btn');
  if(!btn)return;
  btn.classList.add('sol-scrolling');
  clearTimeout(solScrollTimer);
  solScrollTimer=setTimeout(function(){btn.classList.remove('sol-scrolling');},500);
},{passive:true});
