...
// Detecta lacunas e sugere ajuste via IA para um dia
async function ajustarDia(data, eventosDia, escalaBase) {
  // Detecta quais eventos não têm cobertura
...
export default async function handler(req, res) {
  ...
  // Descobre turno predominante de cada pessoa (últimas 3 semanas)
  ...
  if (req.method === 'POST') {
    try {
      const body = req.body||{};
      // Recebe a escala final já calculada do frontend
      const linhasNovas = [];
      for(let i=1;i<=14;i++){
        ...
        ativos.forEach(p=>{
          const t=turnos[p[0]];
          if(!t) return;
          // Verifica se tem ajuste salvo no body
          const key = `${df}|${p[0]}`;
  ...
  // GET — gera proposta com ajustes inteligentes
  const eventos = await getEventosPeriodo(fmtAirtable(inicio), fmtAirtable(fim));

  // Processa cada dia — detecta lacunas e ajusta
  ...
  for(let i=1;i<=14;i++){
    ...
    // Detecta lacunas antes do ajuste
  ...
  // Monta JSON de ajustes para o frontend usar no POST
  const ajustesJSON = {};
...
body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#1c1f26;color:#e2e8f0}
</style>
</head><body>
...
<div style="max-width:1400px;margin:0 auto;padding:16px 20px">
  ...
  <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:16px">
    <div style="background:#242836;border:1px solid #2d3748;border-radius:8px;padding:12px 14px">
      <div style="font-size:9px;color:#718096;font-weight:600;text-transform:uppercase;margin-bottom:4px">Dias gerados</div>
      <div style="font-size:24px;font-weight:700;color:#e2e8f0">14</div>
      <div style="font-size:10px;color:#4a5568;margin-top:2px">${fmtData(inicio)} → ${fmtData(fim)}</div>
    </div>
    <div style="background:#242836;border:1px solid #2d3748;border-radius:8px;padding:12px 14px">
      <div style="font-size:9px;color:#718096;font-weight:600;text-transform:uppercase;margin-bottom:4px">Colaboradores</div>
      <div style="font-size:24px;font-weight:700;color:#e2e8f0">${ativos.filter(p=>turnos[p[0]]).length}</div>
      <div style="font-size:10px;color:#4a5568;margin-top:2px">com turno identificado</div>
    </div>
    <div style="background:${totalAjustes>0?'#1f1a0d':'#242836'};border:1px solid ${totalAjustes>0?'#3d3010':'#2d3748'};border-radius:8px;padding:12px 14px">
      <div style="font-size:9px;color:#718096;font-weight:600;text-transform:uppercase;margin-bottom:4px">Ajustes IA</div>
      <div style="font-size:24px;font-weight:700;color:${totalAjustes>0?'#f6ad55':'#e2e8f0'}">${totalAjustes}</div>
      <div style="font-size:10px;color:#4a5568;margin-top:2px">turnos ajustados para cobrir eventos</div>
    </div>
    <div style="background:${totalLacunas-totalAjustes>0?'#1f1010':'#0d2010'};border:1px solid ${totalLacunas-totalAjustes>0?'#3d2020':'#0d2010'};border-radius:8px;padding:12px 14px">
      <div style="font-size:9px;color:#718096;font-weight:600;text-transform:uppercase;margin-bottom:4px">Lacunas restantes</div>
      <div style="font-size:24px;font-weight:700;color:${totalLacunas-totalAjustes>0?'#fc8181':'#68d391'}">${totalLacunas-totalAjustes}</div>
      <div style="font-size:10px;color:#4a5568;margin-top:2px">${totalLacunas-totalAjustes>0?'eventos sem cobertura':'cobertura completa'}</div>
    </div>
  </div>
  ...
  <div style="background:#242836;border:1px solid #2d3748;border-radius:10px;padding:16px;margin-bottom:16px">
    <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.05em;color:#718096;margin-bottom:10px">✱ Ajustes realizados pela IA</div>
    ${ajustesResumoHtml}
  </div>`:''}
  ...
  <div style="background:#242836;border:1px solid #2d3748;border-radius:10px;overflow:hidden;margin-bottom:16px">
    ...
    <div style="overflow-x:auto">
      <table style="width:100%;border-collapse:collapse">
        <thead><tr>
          <th style="padding:6px 10px;text-align:left;font-size:9px;font-weight:600;color:#718096;text-transform:uppercase;background:#1e2230;border-bottom:1px solid #2d3748;min-width:110px">Dia</th>
          ${cabecalho}
        </tr></thead>
  ...
  <div style="background:#242836;border:1px solid #2d3748;border-radius:10px;padding:16px;display:flex;align-items:center;gap:16px">
    <div style="flex:1">
      ...
      <div style="font-size:11px;color:#718096;margin-top:2px">Grava ${ativos.filter(p=>turnos[p[0]]).length*14} linhas na aba Escala · ${totalAjustes} com turno ajustado pela IA · Você pode ajustar depois direto na planilha.</div>
    </div>
