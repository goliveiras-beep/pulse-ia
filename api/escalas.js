  ...
  if (foraDoHorizonte) {
    ...
    const cabecalho = `<tr>
      ...
      ${datas.map(df=>{
        ...
        const thBg = isHoje ? 'var(--today-bg)' : isFds ? 'var(--fds-th)' : 'var(--th)';
        const thColor = isHoje ? '#1d4ed8' : isFds ? 'var(--fds-c)' : '#888';
        const thBorder = isHoje ? '2px solid var(--today-border)' : isFds ? '3px solid var(--fds-border)' : '1px solid var(--th-border)';
