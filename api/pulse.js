export default async function handler(req, res) {
  ...
  if (!event || event.bot_id || event.subtype || !event.text || !event.user) return;
...
Ajuda o time com informações internas, documentos e suporte geral.
...
Use formatação Slack: *negrito*, _itálico_, listas com •`;
  ...
  try {
    ...
    const resposta = data?.candidates?.[0]?.content?.parts?.[0]?.text || "Não consegui processar sua mensagem. Tente novamente.";
