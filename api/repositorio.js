...
async function listarRepositorio({ q = '', folder = 'todos', limit = 100 } = {}) {
  ...
  const docs = [];
  const folders = resolveFolders(folder);

  for (const f of folders) {
    const driveQuery = "'" + f.id + "' in parents and trashed = false";
    ...
    (data.files || []).forEach(file => {
      ...
      docs.push({
        ...
        atualizadoEm: file.modifiedTime || '',
        tamanho: file.size || '',
        categoria: f.label,
        canalSlack: f.slack,
      });
  ...
  const filtrados = termo
    ? docs.filter(d => normalizar(d.nome + ' ' + d.tipo + ' ' + d.categoria + ' ' + d.canalSlack).includes(termo))
    : docs;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.query.format === 'json') {
      return res.status(200).json({ ok: true, rootFolderId: REPOSITORY_ROOT_FOLDER_ID, total: docs.length, docs });
    }
