...
export default async function handler(req, res) {
  ...
  // OAuth: verificar/cadastrar na planilha
  if (session.isOAuth) {
    const { email, nomeGoogle } = session;
    const equipeOAuth = await getSheet('Equipe!A2:L200');
    const usuarioOAuth = equipeOAuth.find(r => (r[9]||'').toLowerCase() === email.toLowerCase());
...
<body><div class="box">
  ...
  <form method="POST" action="/api/app?action=cadastro-oauth">
  ...
  // Cadastro OAuth (novo colaborador)
  if (req.method === 'POST' && action === 'cadastro-oauth') {
    const { email, nome, cpf, rg, nascimento, telefone, endereco } = req.body || {};
    if (!email || !nome || !cpf || !rg || !nascimento || !endereco) {
      res.setHeader('Content-Type', 'text/html; charset=utf-8');
      return res.status(200).send('<p style="color:red;font-family:sans-serif;padding:20px">Preencha todos os campos obrigatórios. <a href="/api/app">Voltar</a></p>');
    }
    try {
      await appendSheet('Equipe!A:L', [[nome, '', '', cpf, rg, nascimento, endereco, '', 'colaborador', email, 'pendente', telefone||'']]);
    } catch(e) { console.error('Cadastro error:', e.message); }
    res.setHeader('Content-Type', 'text/html; charset=utf-8');
    return res.status(200).send(`<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8"><title>Pulse</title><style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:-apple-system,sans-serif;background:#0f1117;min-height:100vh;display:flex;align-items:center;justify-content:center}.box{background:#161920;border:1px solid #2d3748;border-radius:16px;padding:40px;width:400px;text-align:center;color:#e2e8f0}.icon{font-size:48px;margin-bottom:16px}h1{font-size:20px;font-weight:700;margin-bottom:8px}p{font-size:13px;color:#718096;line-height:1.6}</style></head><body><div class="box"><div class="icon">✅</div><h1>Solicitação enviada!</h1><p>Seus dados foram recebidos.<br>Aguarde a aprovação do gestor para acessar o portal.</p></div></body></html>`);
  }
